...
const ruleLabel=(r:PmSetupRow)=>r.storeLevel?"Store coverage":r.matchedTypes?`${r.matchedTypes} equipment type${r.matchedTypes===1?"":"s"}`:"Equipment rule needs review";
...
export async function buildPmSetupSources(repository:OpsRepository,session:OperatorSession,asOf:string,query:Query):Promise<ProgramPageViewModel> {
  ...
  const rows:TableRowViewModel[]=result.items.map(r=>{
    ...
    return {id:r.id,label:r.name,href,cells:[{key:"target",value:target?r.name:r.assetName??(r.hasAssetReference?"Equipment unavailable":storeLabel(r,session)),secondary:r.assetName||r.hasAssetReference?storeLabel(r,session):"Store coverage"},
