...
import { accountingPayload, requireAccountingAccess } from "@/lib/ops/accounting-import";
...
export default async function AccountingImportPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <div className={layout.page}>
    ...
    {session.accessMode !== "authenticated" ? <details className={styles.controlDisclosure}><summary>Demo import and connection details</summary><p>This test adapter uses fictional accounting records. No accounting system is connected. Selecting a customer’s accounting product is still required.</p>
    ...
    {source && delivery ? <section><h2>{delivery.invoiceNumber}</h2><p>{state[source.matchState]} · Source version {source.sourceRevision} · {delivery.voided ? "Voided in accounting" : delivery.paidMinor ? `${money(delivery.paidMinor, delivery.currency)} recorded as paid` : "No payment recorded"}</p>
      ...
      {source.matchState !== "excluded" ? <><p>{sourcePayload?.changes?.reasons.join(" ")}</p><details open={source.matchState === "needs_review"}><summary>{source.matchState === "matched" ? "Change linked work" : "Review and match this invoice"}</summary>{reviewModel ? <AccountingImportReview key={source.id} initial={reviewModel} /> : null}</details></> : <p>This record is excluded from facilities invoice review and spending totals.</p>}
    </section> : null}
  </div>;
}
