...
import { locationEvidenceSchema, publicApiError, publicApiSuccess } from "@/components/ops-public/server-http";
...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  try {
    ...
    return publicApiSuccess(await getPublicOperationsGateway().checkIn(token, command), 201);
  } catch (error) {
