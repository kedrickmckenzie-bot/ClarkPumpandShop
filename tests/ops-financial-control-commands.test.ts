...
describe("directive financial safeguard decisions",()=>{
  it("receives a durable source invoice and turns a trip difference into a review flag only",async()=>{
    ...
    expect(snapshot.invoiceReferences.find((invoice)=>invoice.id===result.invoiceId)).toMatchObject({invoiceNumber:"SUM-DEMO-NEW-1",operatorWorkOrderNumber:"NL-2026-0104",matchStatus:"confirmed",grossAmount:{amountMinor:36_500,currency:"USD"}});
