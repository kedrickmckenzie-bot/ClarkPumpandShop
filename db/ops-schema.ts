...
export const opsWorkOrders = sqliteTable("ops_work_orders", {
  ...
  priority: text("priority").notNull(), status: text("status").notNull(), version: integer("version").notNull().default(0), internalAccountableParty: text("internal_accountable_party").notNull().default("Facilities coordinator"), accountableParty: text("accountable_party").notNull(), nextAction: text("next_action").notNull(), dueAt: text("due_at"), escalationTo: text("escalation_to"),
