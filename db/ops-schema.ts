...
export const opsVendorComplianceAlerts = sqliteTable("ops_vendor_compliance_alerts", {
  id: id(), organizationId: organizationId(), vendorId: text("vendor_id").notNull(), documentId: text("document_id").notNull(), stage: text("stage").notNull(), expiresAt: text("expires_at").notNull(), reminderId: text("reminder_id"), createdAt: createdAt(),
}, (table) => [uniqueIndex("uidx_ops_vendor_compliance_alert_doc_stage").on(table.organizationId, table.documentId, table.stage), index("idx_ops_vendor_compliance_alert_org_vendor_created").on(table.organizationId, table.vendorId, table.createdAt)]);
...
export const opsRequests = sqliteTable("ops_requests", {
  id: id(), organizationId: organizationId(), reference: text("reference").notNull(), storeId: text("store_id").notNull(), reporterName: text("reporter_name").notNull(), reporterEmployeeId: text("reporter_employee_id"), problem: text("problem").notNull(), priority: text("priority").notNull(), status: text("status").notNull(), version: integer("version").notNull().default(0), submittedAt: text("submitted_at").notNull(), acknowledgedAt: text("acknowledged_at"), acknowledgedByActorType: text("acknowledged_by_actor_type"), acknowledgedByActorId: text("acknowledged_by_actor_id"), acknowledgedByActorName: text("acknowledged_by_actor_name"), linkedWorkOrderId: text("linked_work_order_id"), linkedAt: text("linked_at"), linkedByActorType: text("linked_by_actor_type"), linkedByActorId: text("linked_by_actor_id"), linkedByActorName: text("linked_by_actor_name"), convertedWorkOrderId: text("converted_work_order_id"),
...
export const opsWorkOrderVisitHolds = sqliteTable("ops_work_order_visit_holds", {
  id: id(), organizationId: organizationId(), workOrderId: text("work_order_id").notNull(), posture: text("posture").notNull(), status: text("status").notNull(), internalReviewThresholdMinor: integer("internal_review_threshold_minor"), currency: text("currency"), deadlineAt: text("deadline_at").notNull(), version: integer("version").notNull().default(0), claimedVisitId: text("claimed_visit_id"), claimedVendorId: text("claimed_vendor_id"), claimedAt: text("claimed_at"), plannedReviewAppointmentId: text("planned_review_appointment_id"), plannedReviewSelectedAt: text("planned_review_selected_at"), plannedReviewSelectedByMembershipId: text("planned_review_selected_by_membership_id"), createdByMembershipId: text("created_by_membership_id"), createdByName: text("created_by_name").notNull(), createdAt: createdAt(), updatedAt: text("updated_at").notNull(),
}, (table) => [uniqueIndex("uidx_ops_work_order_visit_holds_org_work").on(table.organizationId, table.workOrderId), index("idx_ops_work_order_visit_holds_org_status_deadline").on(table.organizationId, table.status, table.deadlineAt)]);
