...
export const opsRoleCapabilityOverrides = pgTable("ops_role_capability_overrides", {
  id: id(),
  organizationId: organizationId(),
  role: text("role").notNull(),
  ...
  unique("uq_ops_role_capability_org_id").on(table.organizationId, table.id),
  uniqueIndex("uidx_ops_role_capability_org_role_cap").on(table.organizationId, table.role, table.capability),
  index("idx_ops_role_capability_org_role").on(table.organizationId, table.role),
  foreignKey({ name: "fk_ops_role_capability_org", columns: [table.organizationId], foreignColumns: [opsOrganizations.id] }),
  foreignKey({ name: "fk_ops_role_capability_member", columns: [table.organizationId, table.updatedByMembershipId], foreignColumns: [opsMemberships.organizationId, opsMemberships.id] }),
  check("chk_ops_role_capability_role", sql`${table.role} IN ('facilities_admin', 'regional_manager', 'store_manager')`),
  check("chk_ops_role_capability_cap", sql`${table.capability} IN ('create_work_order', 'issue_work_order', 'confirm_observable_result')`),
  check("chk_ops_role_capability_actor", sql`length(btrim(${table.updatedByName})) > 0`),
]);
...
export const opsWorkflowPolicies = pgTable("ops_workflow_policies", {
  ...
  uniqueIndex("uidx_ops_workflow_policy_active").on(table.organizationId).where(sql`${table.status} = 'active'`),
  foreignKey({ name: "fk_ops_workflow_policy_org", columns: [table.organizationId], foreignColumns: [opsOrganizations.id] }),
  foreignKey({ name: "fk_ops_workflow_policy_member", columns: [table.organizationId, table.createdByMembershipId], foreignColumns: [opsMemberships.organizationId, opsMemberships.id] }),
  check("chk_ops_workflow_policy_status", sql`${table.status} IN ('active', 'superseded')`),
  check("chk_ops_workflow_policy_version", sql`${table.version} > 0`),
]);
...
export const opsWorkOrderVerifications = pgTable("ops_work_order_verifications", {
  ...
  check("chk_ops_work_verifications_reason", sql`${table.decision} = 'verified' OR (${table.reason} IS NOT NULL AND length(btrim(${table.reason})) > 0)`),
