...
export function WorkOrderList() {
  ...
  const rows = useMemo(() => demoData.workOrders.filter((workOrder) => {
    const store = demoData.stores.find((item) => item.id === workOrder.storeId);
    const haystack = `${workOrder.number} ${workOrder.title} ${store?.name ?? ""}`.toLowerCase();
  ...
  }).sort((a, b) => b.createdAt.localeCompare(a.createdAt)), [query, status, region, category]);
  ...
  return <AppShell><div className="page">
    ...
    <PageHeader eyebrow="Internal source of truth" title="Work orders" description="Every request, vendor touch, visit, document, control action and cost remains tied to Clark’s work order.">
      <Link className="button primary" href="/reports/new"><Plus />New report</Link>
      <button className="button" onClick={() => window.print()}><Download />Export view</button>
    </PageHeader>
    <div className="filter-bar">
      <label className="filter-field"><span>Search</span><div style={{ position: "relative" }}><Search size={13} style={{ position: "absolute", left: 9, top: 10, color: "#7a8987" }} /><input style={{ paddingLeft: 29, width: 210 }} value={query} onChange={(event) => reset(() => setQuery(event.target.value))} placeholder="Number, title or store" /></div></label>
      ...
      <table className="data-table"><thead><tr><th>Work order</th><th>Store</th><th>Classification</th><th>Priority</th><th>Status</th><th>Accountable / next action</th><th>Due</th><th className="numeric">Exposure</th></tr></thead>
        <tbody>{visible.map((workOrder) => {
          ...
          return <tr key={workOrder.id}>
            <td><Link className="row-link mono" href={`/work-orders/${workOrder.id}`}>{workOrder.number}</Link><span className="subtext">{workOrder.title}</span></td>
            <td><Link className="row-link" href={`/stores/${store?.id}`}>{store?.name}</Link><span className="subtext">{store?.city}, {store?.state}</span></td>
            ...
            <td><strong>{workOrder.accountableParty}</strong><span className="subtext">{workOrder.nextAction}</span></td>
            <td>{workOrder.dueAt ? formatDate(workOrder.dueAt, true) : "—"}</td>
            <td className="numeric"><strong>{formatCurrency(workOrder.costExposureCents)}</strong></td>
          </tr>;
        })}</tbody>
      </table>
