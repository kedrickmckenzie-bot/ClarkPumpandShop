...
export function CreatedStoreDetail({ storeId }: { storeId: string }) {
  ...
  return <AppShell><div className="page"><Breadcrumbs items={[{ label: "Stores", href: "/stores" }, { label: `Store ${store.code}` }]} /><PageHeader eyebrow={`${store.district || "Independent"} · ${platformData.regions.find((item) => item.id === store.regionId)?.name ?? "No region"}`} title={String(store.name)} description={`${store.address1}, ${store.city}, ${store.state} ${store.postalCode} · ${store.managerName || "Manager unassigned"}`}><Link className="button primary" href={`/work-orders/new?storeId=${storeId}`}><Plus />Create work order</Link></PageHeader>
...
export function CreatedSystemDetail({ storeId, systemId }: { storeId: string; systemId: string }) {
  const { data, error } = useRegistry();
  if (!data) return <LoadingRecord error={error} />;
  const center = data.centers.find((item) => item.id === systemId && item.storeId === storeId);
  if (!center) return <LoadingRecord error="System or equipment group not found in this store." />;
  const store = data.stores.find((item) => item.id === storeId) ?? platformData.stores.find((item) => item.id === storeId);
  const assets = data.assets.filter((item) => item.storeSystemId === systemId);
  const assetIds = new Set(assets.map((item) => item.id));
  const components = data.components.filter((item) => assetIds.has(String(item.assetId)));
  const work = data.workOrders.filter((item) => item.systemId === systemId);
  const category = platformData.categories.find((item) => item.id === center.categoryId);
  return <AppShell><div className="page"><Breadcrumbs items={[{ label: "Stores", href: "/stores" }, { label: `Store ${store?.code}`, href: `/stores/${storeId}` }, { label: String(center.name) }]} /><PageHeader eyebrow={`${category?.name ?? "Maintenance"} · ${center.code}`} title={String(center.name)} description={`${center.location || "Location not specified"} · ${center.description || "Maintainable equipment group"}`}><Link className="button primary" href={`/work-orders/new?storeId=${storeId}&categoryId=${center.categoryId}&systemId=${systemId}`}><Plus />Create work order</Link></PageHeader><div className="metric-grid"><MetricCard label="Registered assets" value={String(assets.length)} note={`${components.length} serviceable components`} icon={Factory} tone={assets.length ? "good" : "warning"} /><MetricCard label="Linked work" value={String(work.length)} note={`${work.filter((item) => !["closed", "cancelled"].includes(String(item.status))).length} currently open`} icon={Wrench} /><MetricCard label="Annual budget" value={formatCurrency(Number(center.annualBudgetCents) || 0)} note={`GL ${center.glCode || "not mapped"}`} icon={CircleDollarSign} /><MetricCard label="Maintenance strategy" value={String(center.maintenanceStrategy || "preventive").replaceAll("_", " ")} note={String(center.ownerName || "Facilities")} icon={ShieldCheck} /></div><section className="panel panel-pad"><PanelTitle title="Equipment in this group" description="Created records stay connected to their store and work-order history." />{assets.map((asset) => <Link className="created-tree-asset" href={`/assets/${asset.id}`} key={asset.id}><Factory /><span><strong>{asset.assetTag} · {asset.name}</strong><small>{asset.manufacturer} {asset.model} · {asset.location}</small></span><b>{components.filter((item) => item.assetId === asset.id).length} parts</b></Link>)}{!assets.length && <div className="empty-state">No individual equipment is required yet. Work can remain at this system level.</div>}</section></div></AppShell>;
}
...
export function CreatedAssetDetail({ assetId }: { assetId: string }) {
  ...
  const center = data.centers.find((item) => item.id === asset.storeSystemId) ?? platformData.systems.find((item) => item.id === asset.storeSystemId);
  const store = data.stores.find((item) => item.id === center?.storeId) ?? platformData.stores.find((item) => item.id === center?.storeId);
...
export function CreatedComponentDetail({ componentId }: { componentId: string }) {
  ...
  const asset = data.assets.find((item) => item.id === component.assetId) ?? platformData.assets.find((item) => item.id === component.assetId);
  const center = data.centers.find((item) => item.id === asset?.storeSystemId) ?? platformData.systems.find((item) => item.id === asset?.storeSystemId);
  const store = data.stores.find((item) => item.id === center?.storeId) ?? platformData.stores.find((item) => item.id === center?.storeId);
