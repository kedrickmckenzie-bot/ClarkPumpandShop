      ...
      <div className="pf-page maintenance-page work-record-page">
        ...
        <section className="maintenance-panel" id="work-source">
          ...
          <div className="work-source-grid">
            ...
            <article className="work-assignment-card"><span>Who is handling the work</span><h3>{workOrder.assignmentType?.replaceAll("_", " ") ?? "Unassigned"}</h3><dl><div><dt>Internal assignee</dt><dd>{workOrder.assignedToName ?? (workOrder.assignmentType === "internal" ? workOrder.accountableParty : "None")}</dd></div><div><dt>Outside vendor</dt><dd>{vendor ? <Link href={`/providers/${vendor.id}`}>{vendor.name}</Link> : "None"}</dd></div><div><dt>Vendor response</dt><dd>{workOrder.vendorAcceptance.replaceAll("_", " ")}</dd></div><div><dt>Response due</dt><dd>{workOrder.targetResponseAt ? formatDate(workOrder.targetResponseAt, true) : "Not set"}</dd></div><div><dt>Work due</dt><dd>{workOrder.targetCompletionAt ? formatDate(workOrder.targetCompletionAt, true) : workOrder.dueAt ? formatDate(workOrder.dueAt, true) : "Not set"}</dd></div></dl><p>{workOrder.assignmentType === "vendor" || workOrder.assignmentType === "blended" ? "The vendor can keep using its own dispatch process. The customer record still captures responses, visits, results, and proof." : "Internal and vendor work use the same problem details, proof, and closing steps."}</p></article>
          </div>
