  ...
  return (
    <AppShell>
      <div className="pf-page maintenance-page">
        <PlatformBreadcrumbs items={[{ label: "Maintenance" }, { label: "Control center" }]} />
        <PlatformPageHeader eyebrow="Maintenance command center" title="Run the work, then trust the reporting." description="A manager-first view of intake, assignment, field progress, blocked work, verification and preventive maintenance across the operating portfolio.">
          <Link className="pf-button" href="/requests"><Inbox />Review requests</Link>
        ...
        <section className="maintenance-manager-brief">
          <div><span className="maintenance-live-label"><span />As of {formatDate(PLATFORM_NOW, true)}</span><h2>{needsDecision.length + blocked.length + closeout.length} work orders need a management decision, blocker cleared or closeout action.</h2><p>Every card and count opens the underlying Clark&apos;s work-order record. Internal, outside-vendor and blended work share the same controls.</p></div>
          <div className="maintenance-brief-signals"><Link href="/work-orders?priority=critical"><strong>{critical.length}</strong><span>critical open</span><ArrowRight /></Link><Link href="/work-orders?queue=overdue"><strong>{overdue.length}</strong><span>past due</span><ArrowRight /></Link><Link href="/pm?status=missed"><strong>{pmExceptions.length}</strong><span>PM exceptions</span><ArrowRight /></Link></div>
        </section>

        <section className="maintenance-stat-grid" aria-label="Maintenance control metrics">
          <PlatformStat label="Needs triage / assignment" value={String(needsDecision.length)} note="Scope, ownership or provider response needs action" icon={Inbox} href="/work-orders?view=decision" tone={needsDecision.length ? "warning" : "positive"} />
          <PlatformStat label="Work in motion" value={String(inMotion.length)} note="Accepted, onsite or return visit scheduled" icon={Wrench} href="/work-orders?view=in-motion" tone="info" />
          <PlatformStat label="Blocked work" value={String(blocked.length)} note={`${overdueFollowUps.length} overdue follow-ups included`} icon={AlertTriangle} href="/work-orders?view=blocked" tone={blocked.length ? "critical" : "positive"} />
          <PlatformStat label="Closeout queue" value={String(closeout.length)} note="Store verification or invoice package remains" icon={ClipboardCheck} href="/work-orders?view=closeout" tone={closeout.length ? "warning" : "positive"} />
        </section>

        <section className="maintenance-stage-board">
          <PlatformSectionHeader title="Work-order control flow" description="Move exceptions through the internal record without replacing vendor or internal-team workflows." href="/work-orders" linkLabel="Open all work" />
          <div className="maintenance-stage-columns">
            <MaintenanceStage title="Triage & assign" count={needsDecision.length} tone="warning" workOrders={needsDecision.slice(0, 5)} />
            <MaintenanceStage title="In motion" count={inMotion.length} tone="info" workOrders={inMotion.slice(0, 5)} />
            <MaintenanceStage title="Blocked" count={blocked.length} tone="critical" workOrders={blocked.slice(0, 5)} />
            <MaintenanceStage title="Verify & close" count={closeout.length} tone="neutral" workOrders={closeout.slice(0, 5)} />
          </div>
        </section>

        <section className="maintenance-panel maintenance-attention-panel">
          <PlatformSectionHeader title="Manager attention queue" description="Critical work, missed targets and missing controls sorted for intervention." href="/accountability" linkLabel="Open accountability center" />
          <CompactWorkTable workOrders={attention.slice(0, 15)} />
        </section>

        <section className="maintenance-workflow-grid">
          <MaintenanceWorkflowLink href="/requests" icon={Inbox} title="Request intake" description="Preserve the store's wording, confirm risk and create controlled work." count={platformData.workOrders.filter((workOrder) => ["employee_report", "manager"].includes(workOrder.origin) && isOpenWorkOrder(workOrder)).length} />
          <MaintenanceWorkflowLink href="/work-orders" icon={ClipboardList} title="Work orders" description="Search every source, classification, assignment, status and due date." count={openWork.length} />
          <MaintenanceWorkflowLink href="/pm" icon={ShieldCheck} title="Preventive maintenance" description="Manage dated occurrences, windows, evidence and missed-work recovery." count={pmExceptions.length} />
          <MaintenanceWorkflowLink href="/my-work" icon={UserRound} title="Internal work queue" description="Field-ready internal assignments with safety, checklist, time and parts context." count={openWork.filter((workOrder) => workOrder.assignmentType === "internal" || workOrder.assignmentType === "blended").length} />
          <MaintenanceWorkflowLink href="/schedule" icon={CalendarClock} title="Due-date schedule" description="See cross-provider commitments without building a dispatch or routing system." count={openWork.filter((workOrder) => Boolean(workOrder.dueAt)).length} />
        </section>
      </div>
    </AppShell>
  );
...
function MaintenanceStage({ title, count, tone, workOrders }: { title: string; count: number; tone: "warning" | "info" | "critical" | "neutral"; workOrders: WorkOrder[] }) {
  return <section className={`maintenance-stage maintenance-stage-${tone}`}><header><span /><strong>{title}</strong><b>{count}</b></header><div>{workOrders.map((workOrder) => <MaintenanceWorkCard key={workOrder.id} workOrder={workOrder} />)}{!workOrders.length && <p className="maintenance-stage-empty">No work in this stage.</p>}</div></section>;
}

function MaintenanceWorkCard({ workOrder }: { workOrder: WorkOrder }) {
  return <Link className="maintenance-work-card" href={`/work-orders/${workOrder.id}`}><div><span>{workOrder.number}</span><PlatformBadge tone={priorityTone(workOrder.priority)}>{priorityLabel[workOrder.priority]}</PlatformBadge></div><h3>{workOrder.title}</h3><p>{workStoreLabel(workOrder)} · {workProviderName(workOrder)}</p><footer><span>{workOrder.accountableParty}</span><small>{workOrder.dueAt ? formatDate(workOrder.dueAt, true) : "Due date missing"}</small></footer></Link>;
}

function MaintenanceWorkflowLink({ href, icon: Icon, title, description, count }: { href: string; icon: typeof Inbox; title: string; description: string; count: number }) {
  return <Link className="maintenance-workflow-link" href={href}><span><Icon /></span><div><strong>{title}</strong><p>{description}</p></div><b>{count}</b><ArrowRight /></Link>;
}
  ...
  return (
    <AppShell>
      <div className="pf-page maintenance-page">
        ...
        <PlatformPageHeader eyebrow="Canonical maintenance record" title="Work orders" description="Search and control every request, assignment, visit, next action, document and cost from the Clark's internal work order.">
          <Link className="pf-button pf-button-primary" href="/work-orders/new"><Plus />Create work order</Link>
        </PlatformPageHeader>

        {scopeLabels.length > 0 && <section className="work-scope-filter"><span>Drill-down scope</span>{scopeLabels.map((label) => <strong key={label}>{label}</strong>)}<button type="button" onClick={clearFilters}>Clear scope</button></section>}
        <section className="work-directory-summary"><span><strong>{rows.length}</strong> matching records</span><span><strong>{rows.filter(isOpenWorkOrder).length}</strong> open</span><span><strong>{rows.filter(isOverdue).length}</strong> past due</span><span><strong>{rows.filter((workOrder) => workOrder.priority === "critical" && isOpenWorkOrder(workOrder)).length}</strong> critical open</span></section>

        <section className="work-filter-panel">
          <label className="work-search-field"><span>Search work, store or equipment</span><div><Search /><input value={query} onChange={(event) => { setQuery(event.target.value); resetPage(); }} placeholder="WO, store number, address, asset tag, assignee or scope" /></div></label>
          ...
          <label className="work-filter-field"><span>Assignment</span><select value={assignment} onChange={(event) => { setAssignment(event.target.value); resetPage(); }}><option value="">All assignments</option><option value="internal">Internal</option><option value="vendor">Outside vendor</option><option value="blended">Blended</option><option value="unassigned">Unassigned</option></select></label>
          ...
          <label className="work-filter-field"><span>Category</span><select value={category} onChange={(event) => { setCategory(event.target.value); resetPage(); }}><option value="">All categories</option>{platformData.categories.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label>
          ...
          <label className="work-filter-field"><span>Provider / assignee</span><select value={provider} onChange={(event) => { setProvider(event.target.value); resetPage(); }}><option value="">All service sources</option><optgroup label="Outside vendors">{platformData.vendors.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</optgroup><optgroup label="Internal technicians">{platformData.technicians.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</optgroup></select></label>
          <label className="work-filter-field"><span>Classification depth</span><select value={depth} onChange={(event) => { setDepth(event.target.value); resetPage(); }}><option value="">Any depth</option><option value="store">Store only</option><option value="category">Category</option><option value="cost-center">Cost center</option><option value="asset">Asset</option><option value="component">Component</option></select></label>
...
function WorkOrderTable({ workOrders, remoteStores = [] }: { workOrders: WorkOrder[]; remoteStores?: RegistryRecord[] }) {
  return <div className="work-table-wrap"><table className="work-table"><thead><tr><th>Work order</th><th>Store</th><th>Scope / classification</th><th>Priority / status</th><th>Assignment</th><th>Accountable next action</th><th>Due</th><th>Exposure</th></tr></thead><tbody>{workOrders.map((workOrder) => {
    const store = workStore(workOrder);
    const remoteStore = remoteStores.find((candidate) => stringValue(candidate, "id") === workOrder.storeId);
    const category = workCategory(workOrder);
    const system = workSystem(workOrder);
    const asset = workAsset(workOrder);
    return <tr key={workOrder.id}><td><Link className="work-record-link" href={`/work-orders/${workOrder.id}`}>{workOrder.number}</Link><span className="work-table-note">{workOrder.title}</span></td><td><Link className="work-record-link" href={`/stores/${workOrder.storeId}`}>{store ? `Store ${store.code} · ${store.city}` : `Store ${stringValue(remoteStore ?? {}, "code", workOrder.storeId)} · ${stringValue(remoteStore ?? {}, "city")}`}</Link><span className="work-table-note">{store?.address1 ?? stringValue(remoteStore ?? {}, "address1")}</span></td><td><strong>{asset?.name ?? system?.name ?? category?.name ?? "Store-level scope"}</strong><span className="work-table-note">{classificationDepth(workOrder)} classified · {workOrder.location || "Location to confirm"}</span></td><td><div className="work-badge-stack"><PlatformBadge tone={priorityTone(workOrder.priority)}>{priorityLabel[workOrder.priority]}</PlatformBadge><PlatformBadge tone={badgeToneForStatus(workOrder.status)}>{workOrderStatusLabel[workOrder.status] ?? workOrder.status.replaceAll("_", " ")}</PlatformBadge></div></td><td><strong>{workProviderName(workOrder)}</strong><span className="work-table-note">{workOrder.assignmentType?.replaceAll("_", " ") ?? "unassigned"}{workOrder.vendorId ? ` · ${workOrder.vendorAcceptance.replaceAll("_", " ")}` : ""}</span></td><td><strong>{workOrder.accountableParty}</strong><span className="work-table-note">{workOrder.nextAction} · escalate to {workOrder.escalation}</span></td><td>{workOrder.dueAt ? <><strong>{formatDate(workOrder.dueAt, true)}</strong>{isOverdue(workOrder) && <span className="work-overdue-note">Past due</span>}</> : <span className="work-overdue-note">Missing</span>}</td><td><strong>{formatCurrency(workOrder.costExposureCents)}</strong><span className="work-table-note">NTE {formatCurrency(workOrder.nteCents)}</span></td></tr>;
  })}</tbody></table></div>;
}
...
function WorkOrderRecordView({ workOrder, context, remoteActivity }: { workOrder: WorkOrder; context: RegistryContext; remoteActivity: ActivityData }) {
  ...
  const closureBlockers = [
    ...
    ...(unreconciledInvoices.length ? [`${unreconciledInvoices.length} invoice allocation${unreconciledInvoices.length === 1 ? " is" : "s are"} unreconciled`] : []),
    ...
    <AppShell>
      <div className="pf-page maintenance-page work-record-page">
        ...
        <header className="work-record-hero">
          ...
          <aside className="work-control-card"><span>Current control owner</span><h2>{workOrder.accountableParty}</h2><p>{workOrder.nextAction}</p><dl><div><dt>Due</dt><dd>{workOrder.dueAt ? formatDate(workOrder.dueAt, true) : "Missing"}</dd></div><div><dt>Escalation</dt><dd>{workOrder.escalation}</dd></div></dl><Link href="/accountability">Open accountability <ArrowRight /></Link></aside>
        </header>

        <nav className="work-record-nav" aria-label="Work-order record sections"><a href="#work-source">Source & scope</a><a href="#work-execution">Execution</a><a href="#work-financials">Financials</a><a href="#work-audit">Audit trail</a></nav>

        <section className="maintenance-stat-grid" aria-label="Work-order totals">
          <PlatformStat label="Recorded cost" value={formatCurrency(spend)} note={`NTE ${formatCurrency(workOrder.nteCents)} · exposure ${formatCurrency(workOrder.costExposureCents)}`} icon={CircleDollarSign} href="#work-financials" />
          ...
          <PlatformStat label="Work plan" value={`${completedChecklist}/${checklist.length}`} note="Required items govern closeout" icon={ClipboardCheck} href="#work-checklist" />
        ...
        <section className="work-close-control">
          ...
          <div><strong>{closureBlockers.length ? "Closure controls are not satisfied" : "Closure controls are satisfied by recorded evidence"}</strong>{closureBlockers.length ? <ul>{closureBlockers.map((blocker) => <li key={blocker}>{blocker}</li>)}</ul> : <p>No required checklist, follow-up, verification or invoice-allocation blocker is visible.</p>}</div>
        </section>

        <section className="maintenance-panel" id="work-source">
          <PlatformSectionHeader title="Source, scope and progressive classification" description="The request remains permanent. Deeper physical associations can be added later without inventing placeholder assets." />
          <div className="work-source-grid">
            <article className="work-scope-card"><span>Requested scope</span><h3>{workOrder.title}</h3><p>{workOrder.description}</p><dl><div><dt>Requested by</dt><dd>{workOrder.requestedBy ?? "Operations"}</dd></div><div><dt>Origin</dt><dd>{workOrder.origin.replaceAll("_", " ")}</dd></div><div><dt>Created</dt><dd>{formatDate(workOrder.createdAt, true)}</dd></div><div><dt>Work type</dt><dd>{workOrder.workType.replaceAll("_", " ")}</dd></div><div><dt>Safety risk</dt><dd>{workOrder.safetyRisk ?? "Not recorded"}</dd></div><div><dt>Access</dt><dd>{workOrder.accessInstructions || "Confirm with store"}</dd></div></dl></article>
            <article className="work-classification-card"><span>Current classification · {classificationDepth(workOrder)}</span><ol><li className="work-classified"><i>1</i><div><small>Store</small><Link href={`/stores/${store.id}`}>{store.name}</Link></div></li><li className={category ? "work-classified" : "work-deferred"}><i>2</i><div><small>Category</small><strong>{category?.name ?? "Deferred"}</strong></div></li><li className={system ? "work-classified" : "work-deferred"}><i>3</i><div><small>Cost center / system</small>{system ? <Link href={`/stores/${store.id}/systems/${system.id}`}>{system.name}</Link> : <strong>Not yet identified</strong>}</div></li><li className={asset ? "work-classified" : "work-deferred"}><i>4</i><div><small>Asset</small>{asset ? <Link href={`/assets/${asset.id}`}>{asset.assetTag} · {asset.name}</Link> : <strong>Deferred until triage or diagnosis</strong>}</div></li><li className={component ? "work-classified" : "work-deferred"}><i>5</i><div><small>Component</small><strong>{component ? `${component.name}${component.partNumber ? ` · ${component.partNumber}` : ""}` : "Optional"}</strong></div></li></ol></article>
            <article className="work-assignment-card"><span>Execution model</span><h3>{workOrder.assignmentType?.replaceAll("_", " ") ?? "Unassigned"}</h3><dl><div><dt>Internal assignee</dt><dd>{workOrder.assignedToName ?? (workOrder.assignmentType === "internal" ? workOrder.accountableParty : "None")}</dd></div><div><dt>Outside provider</dt><dd>{vendor ? <Link href={`/providers/${vendor.id}`}>{vendor.name}</Link> : "None"}</dd></div><div><dt>Vendor response</dt><dd>{workOrder.vendorAcceptance.replaceAll("_", " ")}</dd></div><div><dt>Target response</dt><dd>{workOrder.targetResponseAt ? formatDate(workOrder.targetResponseAt, true) : "Not set"}</dd></div><div><dt>Target completion</dt><dd>{workOrder.targetCompletionAt ? formatDate(workOrder.targetCompletionAt, true) : workOrder.dueAt ? formatDate(workOrder.dueAt, true) : "Not set"}</dd></div></dl><p>{workOrder.assignmentType === "vendor" || workOrder.assignmentType === "blended" ? "The provider retains its own dispatch and technician workflow. Clark's observes response, visits, outcomes and evidence." : "Internal activity uses the same scope, evidence and closeout controls as vendor work."}</p></article>
          </div>
          {reports.map((report) => <article className="work-original-report" key={report.id}><header><LockKeyhole /><div><strong>{report.reference} · immutable employee report</strong><small>{report.reporterName} · {report.reporterRole} · {formatDate(report.submittedAt, true)}</small></div></header><blockquote>{report.originalDescription}</blockquote>{reviews.filter((review) => review.reportId === report.id).map((review) => <div className="work-report-review" key={review.id}><PlatformBadge tone="good">{review.decision}</PlatformBadge><span><strong>{review.reviewerName}</strong><small>{review.context}</small></span></div>)}</article>)}
        </section>

        <div className="work-record-grid" id="work-execution">
          <section className="maintenance-panel" id="work-visits"><PlatformSectionHeader title="Service visits & required follow-up" description="Observed time, location evidence and one basic outcome per visit." />{visits.length ? <div className="work-visit-list">{visits.map((visit, index) => <article className="work-visit-card" key={visit.id}><header><span>Visit {index + 1}</span><strong>{visit.technicianName}</strong><PlatformBadge tone={visit.checkIn.state === "verified" ? "good" : "warning"}>{verificationLabel[visit.checkIn.state]}</PlatformBadge>{visit.outcome && <PlatformBadge tone={visit.outcome === "resolved" || visit.outcome === "no_issue_found" ? "good" : "warning"}>{visitOutcomeLabel[visit.outcome]}</PlatformBadge>}</header><dl><div><dt>Check-in</dt><dd>{formatDate(visit.checkedInAt, true)}</dd></div><div><dt>Distance / accuracy</dt><dd>{visit.checkIn.distanceM ?? "—"}m / {visit.checkIn.accuracyM ?? "—"}m</dd></div><div><dt>Checkout</dt><dd>{visit.checkedOutAt ? formatDate(visit.checkedOutAt, true) : "Active"}</dd></div><div><dt>Checkout evidence</dt><dd>{visit.checkOut ? verificationLabel[visit.checkOut.state] : "Not captured"}</dd></div></dl></article>)}</div> : <PlatformEmpty icon={MapPin} title="No visit evidence" description="No technician check-in or checkout has been recorded for this work order." />}{followUps.map((followUp) => <article className="work-followup-card" key={followUp.id}><FileQuestion /><div><strong>{followUp.nextAction}</strong><p>{followUp.accountableParty} · due {formatDate(followUp.dueAt, true)} · escalate to {followUp.escalation}</p></div><PlatformBadge tone={followUp.status === "open" ? "warning" : "good"}>{followUp.status}</PlatformBadge></article>)}</section>

          <section className="maintenance-panel" id="work-checklist"><PlatformSectionHeader title="Work plan & checklist" description="Required items remain visible for internal, vendor and blended execution." /><div className="work-checklist-summary"><div><strong>{completedChecklist} of {checklist.length} complete</strong><span>{checklist.filter((item) => item.required && !item.completed).length} required items remain</span></div><PlatformProgress value={checklist.length ? completedChecklist / checklist.length : 0} label={checklist.length ? `${Math.round(completedChecklist / checklist.length * 100)}%` : "No plan"} /></div>{checklist.length ? <ol className="work-checklist-list">{checklist.map((item) => <li className={item.completed ? "work-checklist-complete" : "work-checklist-open"} key={item.id}><span>{item.completed ? <CheckCircle2 /> : item.sequence}</span><div><strong>{item.label}</strong><small>{item.required ? "Required" : "Recommended"}{item.completedBy ? ` · ${item.completedBy}` : ""}</small></div></li>)}</ol> : <PlatformEmpty icon={ClipboardCheck} title="No checklist loaded" description="This created work order has no work-plan items available yet." />}</section>

          <section className="maintenance-panel" id="work-notes"><PlatformSectionHeader title="Work notes" description="Timestamped updates retain their intended audience." />{notes.length ? <div className="work-note-list">{notes.map((note) => <article className="work-note" key={note.id}><MessageSquareText /><div><header><strong>{note.author}</strong><PlatformBadge tone={note.visibility === "internal" ? "neutral" : "info"}>{note.visibility}</PlatformBadge></header><small>{note.authorRole} · {formatDate(note.createdAt, true)}</small><p>{note.body}</p></div></article>)}</div> : <PlatformEmpty icon={MessageSquareText} title="No work notes" description="No timestamped notes are available for this record." />}</section>

          <section className="maintenance-panel" id="work-time-parts"><PlatformSectionHeader title="Labor & materials recorded" description="Execution evidence is linked to the work order; it is not a payroll or inventory module." /><div className="work-ledger-split"><div><header><strong>Labor</strong><span>{laborHours.toFixed(2)}h · {formatCurrency(laborValue)}</span></header>{labor.map((entry) => <article className="work-ledger-row" key={entry.id}><Clock3 /><span><strong>{entry.technicianName}</strong><small>{entry.regularHours.toFixed(2)} regular · {entry.overtimeHours.toFixed(2)} overtime · {entry.notes}</small></span><b>{formatCurrency((entry.regularHours + entry.overtimeHours) * entry.hourlyRateCents)}</b></article>)}{!labor.length && <p className="work-ledger-empty">No labor entries.</p>}</div><div><header><strong>Materials</strong><span>{parts.length} lines · {formatCurrency(partsValue)}</span></header>{parts.map((part) => <article className="work-ledger-row" key={part.id}><Package /><span><strong>{part.partNumber || "No part number"} · {part.description}</strong><small>{part.quantity} × {formatCurrency(part.unitCostCents)} · {part.source.replaceAll("_", " ")}</small></span><b>{formatCurrency(part.quantity * part.unitCostCents)}</b></article>)}{!parts.length && <p className="work-ledger-empty">No material entries.</p>}</div></div></section>

          <section className="maintenance-panel" id="work-documents"><PlatformSectionHeader title="Documents & proof" description="Files are classified once and permissioned through the operational record." />{documents.length ? <div className="work-document-grid">{documents.map((document) => <a href={document.href} key={document.id}><FileText /><span><strong>{document.name}</strong><small>{document.classification.replaceAll("_", " ")} · {Math.round(document.bytes / 1024)} KB · {document.visibility.replaceAll("_", " ")}</small></span><ExternalLink /></a>)}</div> : <PlatformEmpty icon={FileText} title="No linked documents" description="No quote, proof, service ticket, invoice or other document is attached." />}</section>
        </div>

        <section className="maintenance-panel" id="work-financials">
          <PlatformSectionHeader title="Quote, authorization, invoice & allocation trail" description="Requested, approved, committed, invoiced, credited and paid values remain separate stages." />
          <div className="work-financial-summary"><article><span>Quotes</span><strong>{formatCurrency(quotes.reduce((sum, quote) => sum + quote.amountCents, 0))}</strong><small>{quotes.length} records</small></article><article><span>Authorized</span><strong>{formatCurrency(authorizations.filter((item) => item.status !== "cancelled").reduce((sum, item) => sum + item.amountCents, 0))}</strong><small>{authorizations.length} records</small></article><article><span>Invoiced</span><strong>{formatCurrency(invoices.filter((item) => item.status !== "void").reduce((sum, item) => sum + item.totalCents, 0))}</strong><small>{invoices.length} records</small></article><article><span>Explicitly allocated</span><strong>{formatCurrency(allocations.reduce((sum, item) => sum + item.amountCents, 0))}</strong><small>{allocations.length} lines</small></article></div>
          <div className="work-financial-grid">
            <div><h3>Commercial events</h3>{quotes.map((quote) => <article className="work-financial-row" key={quote.id}><span><strong>Quote {quote.number}</strong><small>{formatDate(quote.submittedAt)} · {quote.status}</small></span><b>{formatCurrency(quote.amountCents)}</b></article>)}{authorizations.map((authorization) => <article className="work-financial-row" key={authorization.id}><span><strong>Authorization</strong><small>{formatDate(authorization.approvedAt)} · {authorization.status}</small></span><b>{formatCurrency(authorization.amountCents)}</b></article>)}{!quotes.length && !authorizations.length && <p className="work-ledger-empty">No quote or authorization events.</p>}</div>
            <div><h3>Invoice reconciliation</h3>{invoices.map((invoice) => { const totals = allocationTotals(platformData, invoice.id); const lines = allocations.filter((allocation) => allocation.invoiceId === invoice.id); return <article className="work-invoice-card" key={invoice.id}><header><span><strong>{invoice.number}</strong><small>{formatDate(invoice.issuedAt)} · {invoice.status}</small></span><b>{formatCurrency(invoice.totalCents)}</b></header>{lines.map((line) => <div key={line.id}><span>{line.workClass.replaceAll("_", " ")} · {line.costCategory.replaceAll("_", " ")}</span><strong>{formatCurrency(line.amountCents)}</strong></div>)}<footer><span>{totals.unallocated ? `${formatCurrency(totals.unallocated)} unallocated` : "Fully reconciled"}</span><PlatformBadge tone={totals.unallocated ? "critical" : "good"}>{totals.unallocated ? "Blocked" : "Balanced"}</PlatformBadge></footer></article>; })}{!invoices.length && <p className="work-ledger-empty">No invoice records.</p>}</div>
          </div>
        </section>

        <section className="maintenance-panel" id="work-audit">
          <PlatformSectionHeader title="Permanent audit trail" description="Source wording, classification changes, responses, visits, controls and status events append over time." />
          <div className="work-audit-timeline">
            ...
            {!audit.length && !remoteActivity.statuses.length && <PlatformEmpty icon={FileCheck2} title="No detailed audit events available" description="The work order itself is available, but no appended event history was returned." />}
          </div>
        </section>
      </div>
    </AppShell>
