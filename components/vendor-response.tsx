...
export function VendorResponsePage({ token }: { token: string }) {
  ...
  if (!valid) return <div className="mobile-surface"><div className="mobile-frame"><div className="mobile-body"><h1>Link unavailable</h1><p>This response link is invalid or expired. Contact the customer facilities team and reference the work-order number from your email.</p></div></div></div>;
  return <div className="mobile-surface"><div className="mobile-frame"><div className="mobile-head"><div className="mobile-head-row"><div className="brand-copy"><strong>Maintenance Intelligence</strong><span>Vendor work-order response</span></div><Building2 /></div></div><div className="mobile-body">
    {submitted ? <><div className="result-state"><CheckCircle2 size={27} color="#0e6255" /><h1>Response recorded</h1><p>{submitted === "accepted" ? "The customer team has been told your company accepts this work. Dispatch the job in your own system; no technician assignment is needed here." : submitted === "declined" ? "The customer facilities team will retain accountability and select the next action." : "The customer facilities team will review your question and reply through the normal communication channel."}</p></div><Link className="mobile-action secondary" href="/email-outbox">Return to demo outbox</Link></> : <>
      ...
      <div className="form-stack" style={{ marginTop: 14 }}><div className="option-list">{[["accepted", "Accept work", "We will dispatch this in our own system", CheckCircle2], ["clarification", "Request clarification", "We need more scope or access information", HelpCircle], ["declined", "Decline work", "We cannot accept this request", XCircle]].map(([value, label, help, Icon]) => { const SelectionIcon = Icon as typeof CheckCircle2; return <button className="option-card" style={{ textAlign: "left" }} key={String(value)} onClick={() => setResponse(value as Response)}><input type="radio" readOnly checked={response === value} /><span><strong><SelectionIcon size={13} style={{ verticalAlign: "middle", marginRight: 5 }} />{String(label)}</strong><span>{String(help)}</span></span></button>;})}</div>
        {response && response !== "accepted" && <label className="form-field"><span>{response === "declined" ? "Reason" : "Question"}</span><textarea value={note} required onChange={(event) => setNote(event.target.value)} placeholder="Add enough detail for the customer facilities team to act." /></label>}
