...
export function ServiceRunResponseForm({ token, view }: { token: string; view: ServiceRunPublicView }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    const plannedDateValue = requestedStartsAt
      ? isStoreSweep ? requestedStartsAt : new Date(requestedStartsAt).toISOString()
      : undefined;
    try {
      const request = await fetch(`/api/ops-public/service-run/${encodeURIComponent(token)}`, {
        method: "POST", headers: { "content-type": "application/json" },
        body: JSON.stringify({ response, responderName, requestedStartsAt: plannedDateValue, requestedStopOrder, removeWorkOrderIds, reasonCode: needsReason ? reasonCode : undefined, reasonDetail: needsReason ? reasonDetail : undefined }),
      });
  ...
  return (
    <section className={styles.card} aria-labelledby="respond-run-title">
      ...
      <form className={styles.form} onSubmit={submit}>
        ...
        {response === "countered" || isStoreSweep && ["accepted", "work_order_change_requested"].includes(response) ? <label><span className={styles.detailLabel}>{isStoreSweep ? "When does your company plan to visit?" : "Proposed date and time"}</span><input required type="datetime-local" value={requestedStartsAt} onChange={(event) => setRequestedStartsAt(event.target.value)} />{isStoreSweep ? <span className={styles.helper}>Uses {(view.timeZone ?? "the store time zone").replaceAll("_", " ")}. Your company still controls its crew, route, and time onsite.</span> : null}</label> : null}
