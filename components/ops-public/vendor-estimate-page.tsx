...
export function VendorEstimatePage({ token, estimate }: { token: string; estimate: VendorEstimateView }) {
  return (
    <PublicFrame organizationName={estimate.organizationName} context="Vendor Quote Request" mode={estimate.mode}>
      <div className={styles.hero}>
        <div>
          <span className={styles.eyebrow}>Quote requested from {estimate.vendorName}</span>
      ...
      <div className={styles.layout}>
        <div className={styles.stack}>
          <section className={styles.card} aria-labelledby="price-scope-title">
            <div className={styles.cardHeader}>
              <div><span className={styles.eyebrow}>{estimate.requestKindLabel}</span><h2 className={styles.cardTitle} id="price-scope-title">What the operator wants priced</h2></div>
              <FileSearch aria-hidden="true" color="#0d6b62" size={24} />
            </div>
            <p className={styles.problem}>{estimate.problem}</p>
            <div className={styles.callout} style={{ marginTop: "1rem" }}><strong>Requested quote scope</strong><p>{estimate.requestedScope}</p></div>
            <div className={styles.detailGrid} style={{ marginTop: "1.2rem" }}>
              ...
              <div className={styles.detail}><span className={styles.detailLabel}>Quote due</span><p className={styles.detailValue}>{estimate.dueAt ? formatPublicDateTime(estimate.dueAt, estimate.store.timeZone) : "No deadline stated"}</p></div>
            </div>
          </section>
          ...
          {estimate.latestProposal ? (
            <section className={styles.card} aria-labelledby="current-bid-title">
              <div className={styles.cardHeader}>
                <div><span className={styles.eyebrow}>Latest valid quote</span><h2 className={styles.cardTitle} id="current-bid-title">Quote revision {estimate.latestProposal.revision}</h2></div>
                <CircleDollarSign aria-hidden="true" color="#0d6b62" size={24} />
              </div>
              <div className={styles.detailGrid}>
                <div className={styles.detail}><span className={styles.detailLabel}>Quote amount</span><p className={styles.detailValue}>{estimate.latestProposal.amountLabel}</p></div>
          ...
          {estimate.previousProposals.length ? (
            <details className={styles.card}>
              ...
              <div className={styles.stack} style={{ marginTop: "1rem" }}>
                {estimate.previousProposals.map((proposal) => (
        ...
        <aside className={styles.stack} aria-label="Quote-request context">
          ...
          <section className={styles.card}>
            ...
            <h2 className={styles.cardTitle} style={{ marginTop: ".65rem" }}>{estimate.decisionKind === "replacement_quote" ? "This is a replacement quote, not a service assignment" : "This quote request is pricing only"}</h2>
