...
export function ServiceRunResponsePage({ token, view }: { token: string; view: ServiceRunPublicView }) {
  ...
  return (
    <PublicFrame organizationName={view.organizationName} context={isStoreSweep ? "Proposed store visit" : "Vendor Service Run"} mode="demo">
      <div className={styles.hero}>
        <div><span className={styles.eyebrow}>Proposed for {view.vendorName}</span><h1 className={styles.title}>{isStoreSweep ? "Proposed store visit" : "Service Run recommendation"}</h1><p className={styles.lede}>{isStoreSweep ? `${jobCount} approved ${jobCount === 1 ? "job" : "jobs"} at one store` : `${view.stops.length} route stops · ${jobCount} Work Orders`}</p></div>
      ...
      <div className={styles.layout}>
        <div className={styles.stack}>
          <section className={styles.card} aria-labelledby="recommendation-title">
            <div className={styles.cardHeader}><div><span className={styles.eyebrow}>{isStoreSweep ? "Visit summary" : "Explainable recommendation"}</span><h2 className={styles.cardTitle} id="recommendation-title">{isStoreSweep ? "Several approved jobs, one proposed visit" : "Why these obligations are grouped"}</h2></div><Route aria-hidden="true" color="#0d6b62" size={24} /></div>
            <p className={styles.problem}>{view.recommendationExplanation}</p>
            <div className={styles.detailGrid} style={{ marginTop: "1.2rem" }}>
              <div className={styles.detail}><span className={styles.detailLabel}>Proposed visit</span><p className={styles.detailValue}>{formatPublicDateTime(view.proposedStartsAt, view.timeZone)} – {formatPublicDateTime(view.proposedEndsAt, view.timeZone)}</p></div>
              ...
              {isStoreSweep ? <div className={styles.detail}><span className={styles.detailLabel}>Approved jobs</span><p className={styles.detailValue}>{jobCount}</p></div> : <div className={styles.detail}><span className={styles.detailLabel}>Protected route use</span><p className={styles.detailValue}>{view.capacityUsedMinutes} minutes</p></div>}
              <div className={styles.detail}><span className={styles.detailLabel}>{isStoreSweep ? "Time reserved for this visit" : "Estimated presence"}</span><p className={styles.detailValue}>{isStoreSweep ? `${view.estimatedServiceMinutes} minutes for planning` : `${view.estimatedDriveMinutes} travel · ${view.estimatedServiceMinutes} service/buffer`}</p></div>
            </div>
          </section>
          {view.stops.map((stop) => (
            <section className={styles.card} key={stop.id} aria-labelledby={`stop-${stop.id}`}>
              ...
              <div className={styles.detailGrid}>
                <div className={styles.detail}><span className={styles.detailLabel}>Proposed arrival</span><p className={styles.detailValue}>{formatPublicDateTime(stop.proposedArrivalAt, view.timeZone)}</p></div>
                <div className={styles.detail}><span className={styles.detailLabel}>Planned time</span><p className={styles.detailValue}>{isStoreSweep ? `${stop.serviceMinutes} minutes at the store` : `${stop.driveMinutes} travel · ${stop.serviceMinutes} onsite/buffer`}</p></div>
              ...
              {stop.workOrders.map((workOrder) => <div className={styles.callout} style={{ marginTop: ".8rem" }} key={workOrder.id}><strong>{workOrder.number} · {isStoreSweep ? "Approved job" : workOrder.kind}</strong><p>{workOrder.problem}</p><p className={styles.helper}>{isStoreSweep ? `${workOrder.durationMinutes}-minute planning allowance` : `${workOrder.durationMinutes} expected service minutes`}{workOrder.dueWindow ? " · preventive-maintenance window retained" : ""}</p></div>)}
            </section>
          ))}
