...
export function RequestReviewPanel({ model }: { model: RequestReviewViewModel }) {
  ...
  const [decision, setDecision] = useState(model.expectedStatus === "submitted" ? "start_review" : "escalate");
        ...
        <form action={model.submitAction} method="post" onSubmit={submit} className={styles.controlForm}>
          ...
          <label className={styles.field} htmlFor={`request-decision-${model.requestId}`}>
            ...
            <select id={`request-decision-${model.requestId}`} name="decision" required value={decision} onChange={(event) => setDecision(event.target.value)}>
              {model.expectedStatus === "submitted" ? <option value="start_review">Start review</option> : null}
          ...
          <label className={styles.field} htmlFor={`request-note-${model.requestId}`}>
            <span>Decision note{decision === "start_review" ? <small>Optional</small> : <em>Required</em>}</span>
            <textarea
              id={`request-note-${model.requestId}`}
              name="note"
              rows={3}
              required={decision !== "start_review"}
              placeholder={decision === "close" ? "Explain why no work order is needed." : decision === "escalate" ? "Explain what decision or approval is needed." : "Add useful review context."}
            />
          </label>
          ...
          <div className={styles.formFooter}>
            ...
            <button className={styles.primaryButton} type="submit" disabled={state.pending}>{state.pending ? "Recording…" : "Record decision"}<ShieldCheck aria-hidden="true" size={17} /></button>
          </div>
        </form>
      )}
    </section>
