...
export { CreatePmProgramSetupForm } from "./pm-schedule-create";
  ...
  return (
    <div className={styles.formPage}>
      ...
      <form className={styles.recordForm} action={model.submitAction} method="post">
        ...
        {model.coverage ? <section className={styles.formSection}>
          <h2>Equipment at this store</h2>{model.editScheduleHref ? <Link href={model.editScheduleHref}>Edit company schedule</Link> : null}
          <p>All {model.coverage.categoryKey.toUpperCase()} equipment is included automatically. Uncheck exceptions or add other equipment below.</p>
          <label className={styles.choiceCard}><input type="checkbox" name="active" defaultChecked={model.coverage.active} /><span>Include this store in the schedule</span></label>
          {model.coverage.assets.length ? <div className={pmStyles.storeList}>{model.coverage.assets.map(a => <label className={pmStyles.storeRow} key={a.id} aria-label={a.name} htmlFor={`pm-equipment-${a.id}`}><input id={`pm-equipment-${a.id}`} type="checkbox" name="coveredAssetId" value={a.id} defaultChecked={a.included} /><span><strong>{a.name}</strong><small>{a.categoryKey || "Needs classification"}</small></span></label>)}</div> : <p>No equipment recorded. Store service can still be scheduled.</p>}
          <label className={styles.field}><span>Preferred vendor</span><select name="preferredVendorId" defaultValue={model.preferredVendorId ?? ""}><option value="">Choose when assigning work</option>{model.vendors?.map(v => <option key={v.value} value={v.value}>{v.label}</option>)}</select></label>
          <label className={styles.field}><span>Store instructions</span><textarea name="instructions" maxLength={4000} defaultValue={model.instructions} rows={3} /></label>
          {model.masterProgramName ? <button className={styles.secondaryButton} name="mode" value="defaults" formNoValidate>Use schedule defaults</button> : null}
        </section> : null}
        <div className={styles.formNotice}><Info aria-hidden="true" size={20} /><p><strong>Future cycles only.</strong> Issued work and completion records stay the same. Equipment changes apply when the next work order is created.</p></div>
