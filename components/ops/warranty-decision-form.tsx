...
export function WarrantyDecisionForm({ caseId, initialCoverage = "", diagnosis = "", hold = true, returnQueue, returnDecision }: { caseId: string; initialCoverage?: string; diagnosis?: string; hold?: boolean; returnQueue?: string; returnDecision?: string }) {
  ...
  return <form className={styles.form} action={`/api/ops/warranties/${caseId}/decision`} method="post" onSubmit={async (event) => {
    ...
    try {
      ...
      router.push(`/app/warranties/${caseId}?${new URLSearchParams({ updated: "coverage", ...(returnQueue ? { reviewQueue: returnQueue } : {}), ...(returnDecision ? {returnDecision} : {}) })}#diagnosis`);
      router.refresh();
    ...
    {returnDecision ? <input type="hidden" name="returnDecision" value={returnDecision} /> : null}
