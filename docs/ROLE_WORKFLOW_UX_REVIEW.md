...
The September completion pass adds completed-task history, source-by-source grouped expansion, supported **Save and open next** review actions, and files associated with exact quote revisions. See `WORKFLOW_ACCOUNTING_COMPLETION.md` for validation and limits. Advanced confirmed-opportunity/review-window analysis still uses the compatibility snapshot; normal lists and the ordinary held-work path remain bounded and query-first.
