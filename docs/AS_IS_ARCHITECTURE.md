...
| Jobs/delivery | Platform job cycles exist and are slot-idempotent: transactional-outbox delivery (`lib/ops/outbox-delivery.ts`) and SLA escalation (`lib/ops/job-workers.ts`), with a `JobRun` registry table (`ops_job_runs`, unique per organization+job+slot), a Cloudflare Worker `scheduled` entry point, and a PostgreSQL cron script (`npm run jobs:postgres`). Delivery is to the structured operational log only — no external email/SMS/vendor-notification channel. PM recurrence worker and operational monitors remain open. |
