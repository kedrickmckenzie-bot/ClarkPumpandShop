...
| --- | --- | --- |
| Manager home | `loadDashboardModel`, `loadOwnerBriefModel` | Home activity totals, stage counts and the seven-row review preview now use repository queries; replace remaining snapshot chart/lifecycle/spotlight inputs and the owner brief |
...
**The dashboard still loads a tenant snapshot for chart, lifecycle and invoice spotlight inputs.** The full action-center list also retains its snapshot path and expanded source evidence. P5-01/P5-02/P5-03 stay open. The new query is a live partial replacement, not completion of the dashboard migration.
...
Next construct chart presentation from the existing paginated group queries and add lifecycle/estimate aggregate evidence and spotlight queries. Connect these to the dashboard renderer, then retire `sessionAndFixture()` from `loadDashboardModel` only after every input has a bounded replacement. The attention population and native activity totals are already connected; do not rebuild them again.
