...
function buildFixture(): OpsFixture {
  ...
  workOrders.push({ id: activePlumbingWorkId, organizationId: organization.id, number: "CPS-2026-0120", storeId: "store-northline-113", problem: "Prep-sink drain is backing up during the lunch period", authorizedScope: "Clear the accessible drain restriction, confirm normal flow, and report any downstream line condition that needs separate work.", categoryKey: "plumbing", taxonomyNodeId: "taxonomy-northline-plumbing", priority: "urgent", status: "in_progress", version: 0, accountableParty: "ClearFlow HVAC, Plumbing & Kitchen Repair", nextAction: "Record service outcome", dueAt: at(8, 25, 20), escalationTo: "Facilities coordinator", nte: { amountMinor: 120_000, currency: "USD" }, createdAt: at(8, 25, 14, 20) });
  ...
  issuances.push({ id: activePlumbingIssuanceId, organizationId: organization.id, workOrderId: activePlumbingWorkId, assignmentId: activePlumbingAssignmentId, revision: 1, immutablePayloadJson: JSON.stringify({ organizationName: organization.name, workOrderNumber: "CPS-2026-0120", store: { id: "store-northline-113" }, vendor: { id: "vendor-northline-cedar", name: "ClearFlow HVAC, Plumbing & Kitchen Repair" }, problem: "Prep-sink drain is backing up during the lunch period", priority: "urgent", billingInstruction: "Reference operator work order CPS-2026-0120 on all service paperwork and invoices." }), channel: "email", issuedAt: at(8, 25, 14, 35) });
  ...
  recentServiceStories.forEach((story, index) => {
    ...
    workOrders.push({ id: workOrderId, organizationId: organization.id, number: workOrderNumber, storeId: store.id, requestId, problem: story.problem, authorizedScope: `Diagnose the reported ${story.categoryKey} issue, complete authorized work, and record the observed outcome.`, categoryKey: story.categoryKey, taxonomyNodeId: taxonomyByCategory[story.categoryKey], assetId, priority: story.priority, status: story.workStatus, accountableParty: story.repairEstimateMinor ? "Facilities coordinator" : story.providerName, nextAction: open ? story.followUpAction! : "No action required", dueAt: open ? story.followUpDueAt : undefined, escalationTo: open ? "Clark Pump and Shop Facilities" : undefined, nte, repairEstimate: story.repairEstimateMinor ? { amountMinor: story.repairEstimateMinor, currency: "USD" } : undefined, estimatedServiceExtensionMonths: story.estimatedServiceExtensionMonths, vendorServiceTicketNumber: vendor ? `${vendor.code.toUpperCase()}-AUG-${story.storeNumber}-${index + 1}` : undefined, createdAt, closedAt: open ? undefined : checkedOutAt });
