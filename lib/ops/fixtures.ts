...
function buildFixture(): OpsFixture {
  ...
  const equipmentTemplates: EquipmentTemplate[] = [
    ...
    ["electrical-distribution", "taxonomy-northline-electrical", "Store electrical distribution", 30],
    ["exterior-lighting", "taxonomy-northline-canopy_lighting", "Exterior lighting system", 20],
    ["plumbing-fixture", "taxonomy-northline-plumbing", "Commercial plumbing fixture", 20],
  ...
  addTemplateComponents("electrical-distribution", [["panel", "Distribution panel"], ["main-breaker", "Main breaker", "panel"], ["branch-circuit", "Branch circuit", "panel"], ["receptacles", "Receptacles", "branch-circuit"], ["grounding", "Grounding and bonding", "panel"]]);
  addTemplateComponents("exterior-lighting", [["feed-circuit", "Electrical feed circuit"], ["disconnect", "Service disconnect", "feed-circuit"], ["lighting-circuit", "Lighting circuit", "feed-circuit"], ["fixtures", "Lighting fixtures", "lighting-circuit"], ["controls", "Photocell and lighting controls", "lighting-circuit"]]);
  ...
  const showcaseBuildingAssets: Array<[Asset, string]> = [
    [{ id: "asset-103-stockroom-electrical", organizationId: organization.id, storeId: "store-northline-103", categoryKey: "electrical", taxonomyNodeId: "taxonomy-northline-electrical", groupPath: ["Electrical", "Interior distribution"], assetTag: "103-ELEC-01", name: "Stockroom electrical circuit", manufacturer: "Square D", model: "QO branch distribution", serialNumber: "QD103BR2018", supplier: "BrightLine Electrical & Lighting", installedAt: atYear(2018, 5, 12), expectedLifeYears: 30, replacementPlanningExcludedAt: at(1, 5, 14), replacementPlanningExclusionReason: "Building electrical infrastructure is maintained as-needed and is not included in equipment replacement forecasting.", status: "operational", createdAt: at(1, 5, 14) }, "electrical-distribution"],
    [{ id: "asset-108-canopy-lighting", organizationId: organization.id, storeId: "store-northline-108", categoryKey: "electrical", taxonomyNodeId: "taxonomy-northline-canopy_lighting", groupPath: ["Electrical", "Exterior lighting", "Fuel canopy"], assetTag: "108-ELEC-02", name: "Fuel canopy lighting - all islands", manufacturer: "LSI Industries", model: "LED canopy system", serialNumber: "LSI108CP2019", supplier: "BrightLine Electrical & Lighting", installedAt: atYear(2019, 8, 20), expectedLifeYears: 20, replacementPlanningExcludedAt: at(1, 5, 14), replacementPlanningExclusionReason: "Lighting circuits are repaired by component and are not forecast as a single equipment replacement.", status: "watch", createdAt: at(1, 5, 14) }, "exterior-lighting"],
    [{ id: "asset-104-roadside-sign-lighting", organizationId: organization.id, storeId: "store-northline-104", categoryKey: "electrical", taxonomyNodeId: "taxonomy-northline-canopy_lighting", groupPath: ["Electrical", "Exterior lighting", "Roadside sign"], assetTag: "104-ELEC-03", name: "Roadside price-sign lighting", manufacturer: "Watchfire", model: "LED price-sign illumination", serialNumber: "WF104PS2020", supplier: "BrightLine Electrical & Lighting", installedAt: atYear(2020, 4, 18), expectedLifeYears: 20, replacementPlanningExcludedAt: at(1, 5, 14), replacementPlanningExclusionReason: "Sign lighting is repaired by component and is not forecast as a single equipment replacement.", status: "watch", createdAt: at(1, 5, 14) }, "exterior-lighting"],
    [{ id: "asset-114-canopy-lighting", organizationId: organization.id, storeId: "store-northline-114", categoryKey: "electrical", taxonomyNodeId: "taxonomy-northline-canopy_lighting", groupPath: ["Electrical", "Exterior lighting", "Fuel canopy"], assetTag: "114-ELEC-02", name: "Fuel canopy lighting - west island", manufacturer: "LSI Industries", model: "LED canopy system", serialNumber: "LSI114CP2017", supplier: "BrightLine Electrical & Lighting", installedAt: atYear(2017, 9, 14), expectedLifeYears: 20, replacementPlanningExcludedAt: at(1, 5, 14), replacementPlanningExclusionReason: "Lighting circuits are repaired by component and are not forecast as a single equipment replacement.", status: "out_of_service", createdAt: at(1, 5, 14) }, "exterior-lighting"],
    [{ id: "asset-111-restroom-hand-sink", organizationId: organization.id, storeId: "store-northline-111", categoryKey: "plumbing", taxonomyNodeId: "taxonomy-northline-plumbing", groupPath: ["Plumbing", "Restroom fixtures"], assetTag: "111-PLMB-01", name: "Customer restroom hand sink", manufacturer: "American Standard", model: "Commercial wall-hung sink", serialNumber: "AS111HS2019", supplier: "ClearFlow HVAC, Plumbing & Kitchen Repair", installedAt: atYear(2019, 6, 10), expectedLifeYears: 20, replacementPlanningExcludedAt: at(1, 5, 14), replacementPlanningExclusionReason: "Standard plumbing fixtures are repaired as needed and excluded from capital equipment forecasting.", status: "operational", createdAt: at(1, 5, 14) }, "plumbing-fixture"],
    [{ id: "asset-113-prep-sink", organizationId: organization.id, storeId: "store-northline-113", categoryKey: "plumbing", taxonomyNodeId: "taxonomy-northline-plumbing", groupPath: ["Plumbing", "Foodservice fixtures"], assetTag: "113-PLMB-01", name: "Foodservice prep sink", manufacturer: "Advance Tabco", model: "Commercial three-compartment sink", serialNumber: "AT113PS2021", supplier: "ClearFlow HVAC, Plumbing & Kitchen Repair", installedAt: atYear(2021, 3, 8), expectedLifeYears: 20, replacementPlanningExcludedAt: at(1, 5, 14), replacementPlanningExclusionReason: "Standard plumbing fixtures are repaired as needed and excluded from capital equipment forecasting.", status: "watch", createdAt: at(1, 5, 14) }, "plumbing-fixture"],
  ];
  showcaseBuildingAssets.forEach(([asset, templateKey]) => seedAsset(asset, templateKey));
  ...
  stores.forEach((store, storeIndex) => {
    for (let historyIndex = 0; historyIndex < 4; historyIndex += 1) {
      // Place the completed repair history across the previously thin
      // Jan–Jul 2025 window while preserving Store 104's established recent
      // compressor story.
      // The stagger is intentionally store-specific: these are observed
      // failures, not a fabricated companywide maintenance cadence. Store 104
      // keeps its established four-record compressor story intact.
      const monthOffset = store.storeNumber === "104"
        ? (storeIndex * 4 + historyIndex) % 20
        : 1 + ((storeIndex * 2 + historyIndex * 3) % 7);
  ...
  const recurringServiceSeeds: ReadonlyArray<{
    ...
    // Store-specific 2025 events add ordinary weather and drain work without
    // fabricating a uniform cadence or a summary total.
    ...
    // Portfolio programs are represented in both years so the historical
    // benchmark sees the ordinary seasonal workload that produced the recent
    // records. These remain source-level work orders and visits, not totals.
    { key: "grounds-spring-2025", year: 2025, month: 5, day: 8, categoryKey: "exterior", taxonomyNodeId: "taxonomy-northline-landscaping", vendorId: "vendor-northline-four-seasons", problem: "Scheduled spring grounds cleanup and landscape bed service", scope: "Complete scheduled grounds service and document site condition before departure.", amountMinor: 46_500 },
    { key: "hvac-cooling-2025", year: 2025, month: 6, day: 16, categoryKey: "hvac", taxonomyNodeId: "taxonomy-northline-rooftop_units", vendorId: "vendor-northline-cedar", problem: "Seasonal rooftop-unit filter and cooling inspection", scope: "Replace filters, clear drains, inspect belts, and record cooling-readiness condition.", amountMinor: 64_500 },
    { key: "food-safety-2025", year: 2025, month: 4, day: 21, categoryKey: "foodservice", taxonomyNodeId: "taxonomy-northline-ovens", vendorId: "vendor-northline-cedar", problem: "Scheduled hot-food equipment cleaning and safety inspection", scope: "Complete scheduled cleaning and inspection; report repair needs separately.", amountMinor: 56_500 },
  ...
  // A completed no-WO visit preserves the common direct-call path: a store
  // manager called a trusted vendor before an operator record existed. The
  // observed visit and technician checkout notes remain available so facilities
  // can create the canonical work order after service without backdating it.
  const noWoStart = at(8, 25, 13, 40);
  const noWoEnd = at(8, 25, 14, 35);
  visits.push({ id: NORTHLINE_DEMO_HANDLES.unmatchedVisitId, organizationId: organization.id, storeId: "store-northline-107", providerKind: "outside_vendor", vendorId: "vendor-northline-brightpath", unmatchedReason: "Store manager called the vendor directly; no work order had been created before arrival", technicianName: "Nolan Reed", providerName: "BrightLine Electrical & Lighting", purpose: "Restore the flickering stockroom light and inspect the loose junction-box cover", status: "checked_out", startedChannel: "qr", endedChannel: "store_device", checkedInAt: noWoStart, checkedOutAt: noWoEnd, outcome: "resolved", outcomeNotes: "Replaced the failed LED driver, secured the junction-box cover, and confirmed the stockroom light remained stable before departure.", observedDurationSeconds: durationSeconds(noWoStart, noWoEnd) });
  visitEvidence.push(
    { id: "evidence-visit-northline-107-no-wo-in", organizationId: organization.id, visitId: NORTHLINE_DEMO_HANDLES.unmatchedVisitId, kind: "check_in", channel: "qr", observedAt: noWoStart, location: { result: "verified", accuracyM: 18, distanceM: 24, capturedAt: noWoStart }, payloadJson: JSON.stringify({ workOrderSelection: "no_work_order_provided", reason: "Store manager called the vendor directly" }) },
    { id: "evidence-visit-northline-107-no-wo-out", organizationId: organization.id, visitId: NORTHLINE_DEMO_HANDLES.unmatchedVisitId, kind: "check_out", channel: "store_device", observedAt: noWoEnd, location: { result: "trusted_store_device", capturedAt: noWoEnd }, payloadJson: JSON.stringify({ outcome: "resolved", outcomeNotes: "Replaced the failed LED driver, secured the junction-box cover, and confirmed the stockroom light remained stable before departure." }) },
  );
  exceptions.push({ id: "exception-northline-107-no-wo", organizationId: organization.id, kind: "no_work_order", storeId: "store-northline-107", visitId: NORTHLINE_DEMO_HANDLES.unmatchedVisitId, vendorId: "vendor-northline-brightpath", severity: "attention", status: "open", summary: "Completed direct-call visit needs an operator work order", detectedAt: noWoEnd });

  // Keep the live-operations view distinct from the completed direct-call
  // example above. This is ordinary issued work that is currently onsite.
  const activePlumbingWorkId = "wo-current-113-prep-sink";
  const activePlumbingAssignmentId = "assignment-current-113-prep-sink";
  const activePlumbingIssuanceId = "issuance-current-113-prep-sink-r1";
  const activePlumbingVisitId = "visit-current-113-prep-sink";
  const activePlumbingStart = at(8, 25, 15, 50);
  const activePlumbingRequestId = "request-current-113-prep-sink";
  requests.push({ id: activePlumbingRequestId, organizationId: organization.id, reference: "REQ-26-113P", storeId: "store-northline-113", reporterName: "Finley Ross", reporterEmployeeId: "E6113", problem: "Prep-sink drain is backing up during the lunch period", priority: "urgent", status: "converted", submittedAt: at(8, 25, 14), convertedWorkOrderId: activePlumbingWorkId });
  workOrders.push({ id: activePlumbingWorkId, organizationId: organization.id, number: "CPS-2026-0120", storeId: "store-northline-113", requestId: activePlumbingRequestId, problem: "Prep-sink drain is backing up during the lunch period", authorizedScope: "Clear the accessible drain restriction, confirm normal flow, and report any downstream line condition that needs separate work.", categoryKey: "plumbing", taxonomyNodeId: "taxonomy-northline-plumbing", assetId: "asset-113-prep-sink", componentId: "component-113-prep-sink-drain", priority: "urgent", status: "in_progress", version: 0, accountableParty: "ClearFlow HVAC, Plumbing & Kitchen Repair", nextAction: "Record service outcome", dueAt: at(8, 25, 20), escalationTo: "Facilities coordinator", nte: { amountMinor: 120_000, currency: "USD" }, createdAt: at(8, 25, 14, 20) });
  assignments.push({ id: activePlumbingAssignmentId, organizationId: organization.id, workOrderId: activePlumbingWorkId, kind: "outside_vendor", vendorId: "vendor-northline-cedar", status: "accepted", assignedAt: at(8, 25, 14, 30) });
  issuances.push({ id: activePlumbingIssuanceId, organizationId: organization.id, workOrderId: activePlumbingWorkId, assignmentId: activePlumbingAssignmentId, revision: 1, immutablePayloadJson: JSON.stringify({ organizationName: organization.name, workOrderNumber: "CPS-2026-0120", store: { id: "store-northline-113" }, vendor: { id: "vendor-northline-cedar", name: "ClearFlow HVAC, Plumbing & Kitchen Repair" }, problem: "Prep-sink drain is backing up during the lunch period", priority: "urgent", asset: { id: "asset-113-prep-sink", name: "Foodservice prep sink", assetTag: "113-PLMB-01" }, billingInstruction: "Reference operator work order CPS-2026-0120 on all service paperwork and invoices." }), channel: "email", issuedAt: at(8, 25, 14, 35) });
  visits.push({ id: activePlumbingVisitId, organizationId: organization.id, storeId: "store-northline-113", providerKind: "outside_vendor", vendorId: "vendor-northline-cedar", workOrderId: activePlumbingWorkId, technicianName: "Imani Lewis", providerName: "ClearFlow HVAC, Plumbing & Kitchen Repair", purpose: "Restore normal prep-sink drainage", status: "active", startedChannel: "qr", checkedInAt: activePlumbingStart });
  visitEvidence.push({ id: `evidence-${activePlumbingVisitId}-in`, organizationId: organization.id, visitId: activePlumbingVisitId, kind: "check_in", channel: "qr", observedAt: activePlumbingStart, location: { result: "verified", accuracyM: 16, distanceM: 19, capturedAt: activePlumbingStart }, payloadJson: JSON.stringify({ workOrderNumber: "CPS-2026-0120" }) });
  ...
  const recentServiceStories: RecentServiceStory[] = [
    { key: "aug-101-refrigeration", storeNumber: "101", categoryKey: "refrigeration", assetId: "asset-101-beer-cave", componentId: "component-101-beer-cave-controller", problem: "Beer cave temperature climbed to 44°F during the afternoon rush", providerKind: "outside_vendor", vendorId: "vendor-northline-summit", technicianName: "Chris Walker", providerName: "ColdLine Refrigeration & HVAC", priority: "urgent", submittedAt: at(8, 1, 19, 20), checkedInAt: at(8, 2, 13, 10), durationMinutes: 112, startedChannel: "qr", endedChannel: "secure_link", locationResult: "verified", outcome: "resolved", outcomeNotes: "Cleared a blocked condensate path, replaced the failed controller probe, and documented a stable 36°F box temperature.", workStatus: "completed_pending_review", nteAmountMinor: 185_000, laborAmountMinor: 46_000, materialsAmountMinor: 31_500, followUpAction: "Verify the stable beer-cave temperature and accept or reject the recorded outcome", followUpDueAt: at(8, 25, 16) },
    { key: "aug-102-hvac", storeNumber: "102", categoryKey: "hvac", assetId: "asset-102-rtu-1", componentId: "component-102-rtu-1-condenser-fan", problem: "Sales floor rooftop unit runs continuously but cannot hold setpoint", providerKind: "outside_vendor", vendorId: "vendor-northline-cedar", technicianName: "Sam Patel", providerName: "ClearFlow HVAC, Plumbing & Kitchen Repair", priority: "urgent", submittedAt: at(8, 2, 21, 5), checkedInAt: at(8, 3, 14, 35), durationMinutes: 86, startedChannel: "secure_link", endedChannel: "qr", locationResult: "verified", outcome: "diagnosed_waiting_parts", outcomeNotes: "Confirmed a failed condenser-fan motor and damaged fan shroud. Cooling is limited while management reviews the quoted repair against replacement.", workStatus: "awaiting_approval", nteAmountMinor: 240_000, laborAmountMinor: 39_500, materialsAmountMinor: 0, repairEstimateMinor: 780_000, estimatedServiceExtensionMonths: 36, followUpAction: "Compare the $7,800 fan assembly repair with rooftop-unit replacement", followUpDueAt: at(8, 27, 16) },
    { key: "aug-103-electrical", storeNumber: "103", categoryKey: "electrical", assetId: "asset-103-stockroom-electrical", componentId: "component-103-stockroom-electrical-receptacles", problem: "Manager reported intermittent power loss at the stockroom receptacles", providerKind: "internal", internalMembershipId: "membership-northline-tech-1", technicianName: "Maria Santos", providerName: "Clark Pump and Shop Maintenance", priority: "routine", submittedAt: at(8, 4, 11), checkedInAt: at(8, 4, 14, 5), durationMinutes: 54, startedChannel: "store_device", endedChannel: "store_device", locationResult: "trusted_store_device", outcome: "no_issue_found", outcomeNotes: "Tested the receptacles under load and checked the panel; no fault recurred. Store will monitor and photograph the next event.", workStatus: "closed", nteAmountMinor: 60_000, laborAmountMinor: 21_500, materialsAmountMinor: 0 },
    { key: "aug-105-forecourt", storeNumber: "105", categoryKey: "forecourt", assetId: "asset-105-dispenser-4", componentId: "component-105-dispenser-4-payment-terminal", problem: "Dispenser 4 card reader reboots during some contactless transactions", providerKind: "outside_vendor", vendorId: "vendor-northline-forecourt", technicianName: "Dana Ruiz", providerName: "PumpPro Fuel & Dispenser Repair", priority: "urgent", submittedAt: at(8, 4, 18, 40), checkedInAt: at(8, 5, 12, 50), durationMinutes: 74, startedChannel: "qr", endedChannel: "store_device", locationResult: "verified", outcome: "return_required", outcomeNotes: "Isolated the fault to the payment-terminal communication board. Dispenser remains available for chip transactions pending replacement.", workStatus: "waiting_on_parts", nteAmountMinor: 275_000, laborAmountMinor: 48_000, materialsAmountMinor: 0, followUpAction: "Confirm the ordered board shipment and return-service date", followUpDueAt: at(8, 26, 18) },
    ...
    { key: "aug-108-electrical", storeNumber: "108", categoryKey: "electrical", assetId: "asset-108-canopy-lighting", componentId: "component-108-canopy-lighting-feed-circuit", problem: "Two canopy-lighting circuits remain dark after lamp replacement", providerKind: "outside_vendor", vendorId: "vendor-northline-brightpath", technicianName: "Nolan Reed", providerName: "BrightLine Electrical & Lighting", priority: "routine", submittedAt: at(8, 6, 20, 10), checkedInAt: at(8, 7, 15, 15), durationMinutes: 91, startedChannel: "secure_link", endedChannel: "store_device", locationResult: "low_accuracy", outcome: "unable_to_complete", outcomeNotes: "Confirmed an underground-feed fault. A lift and utility locate are required before repair can proceed safely.", workStatus: "waiting_on_vendor", nteAmountMinor: 225_000, laborAmountMinor: 44_500, materialsAmountMinor: 0, followUpAction: "Send an updated estimate and proposed return date", followUpDueAt: at(8, 27, 20) },
    { key: "aug-110-refrigeration", storeNumber: "110", categoryKey: "refrigeration", assetId: "asset-110-beer-cave", componentId: "component-110-beer-cave-evaporator", problem: "Beer-cave evaporator is icing and airflow is dropping", providerKind: "outside_vendor", vendorId: "vendor-northline-summit", technicianName: "Drew Miller", providerName: "ColdLine Refrigeration & HVAC", priority: "urgent", submittedAt: at(8, 7, 22, 15), checkedInAt: at(8, 8, 11, 40), durationMinutes: 128, startedChannel: "qr", endedChannel: "secure_link", locationResult: "permission_denied", outcome: "temporary_repair", outcomeNotes: "Defrosted the coil and restored airflow. The vendor quoted replacement of the evaporator controls and fan assembly; management must compare the repair with replacement before permanent work.", workStatus: "awaiting_approval", nteAmountMinor: 210_000, laborAmountMinor: 53_000, materialsAmountMinor: 12_500, repairEstimateMinor: 960_000, estimatedServiceExtensionMonths: 24, followUpAction: "Compare the $9,600 evaporator repair with beer-cave replacement", followUpDueAt: at(8, 28, 16) },
    { key: "aug-111-plumbing", storeNumber: "111", categoryKey: "plumbing", assetId: "asset-111-restroom-hand-sink", componentId: "component-111-restroom-hand-sink-supply", problem: "Restroom hand-sink supply connection is leaking into the cabinet", providerKind: "outside_vendor", vendorId: "vendor-northline-cedar", technicianName: "Imani Lewis", providerName: "ClearFlow HVAC, Plumbing & Kitchen Repair", priority: "routine", submittedAt: at(8, 9, 16, 45), checkedInAt: at(8, 10, 12, 15), durationMinutes: 82, startedChannel: "secure_link", endedChannel: "store_device", locationResult: "verified", outcome: "resolved", outcomeNotes: "Replaced the supply stop and flex connector, dried the cabinet, and verified no leakage under repeated use.", workStatus: "completed_pending_review", nteAmountMinor: 125_000, laborAmountMinor: 37_500, materialsAmountMinor: 18_900, followUpAction: "Review the store confirmation and recorded cost, then close the work order", followUpDueAt: at(8, 25, 17) },
  ];
  ...
  recentServiceStories.forEach((story, index) => {
    ...
    workOrders.push({ id: workOrderId, organizationId: organization.id, number: workOrderNumber, storeId: store.id, requestId, problem: story.problem, authorizedScope: `Diagnose the reported ${story.categoryKey} issue, complete authorized work, and record the observed outcome.`, categoryKey: story.categoryKey, taxonomyNodeId: taxonomyByCategory[story.categoryKey], assetId, componentId: story.componentId, priority: story.priority, status: story.workStatus, accountableParty: story.repairEstimateMinor ? "Facilities coordinator" : story.providerName, nextAction: open ? story.followUpAction! : "No action required", dueAt: open ? story.followUpDueAt : undefined, escalationTo: open ? "Clark Pump and Shop Facilities" : undefined, nte, repairEstimate: story.repairEstimateMinor ? { amountMinor: story.repairEstimateMinor, currency: "USD" } : undefined, estimatedServiceExtensionMonths: story.estimatedServiceExtensionMonths, vendorServiceTicketNumber: vendor ? `${vendor.code.toUpperCase()}-AUG-${story.storeNumber}-${index + 1}` : undefined, createdAt, closedAt: open ? undefined : checkedOutAt });
    ...
    if (open) followUps.push({ id: `follow-up-recent-${story.key}`, organizationId: organization.id, workOrderId, sourceVisitId: visitId, accountableParty: story.repairEstimateMinor ? "Facilities coordinator" : story.providerName, nextAction: story.followUpAction!, dueAt: story.followUpDueAt!, escalationTo: "Clark Pump and Shop Facilities", status: "open", createdAt: checkedOutAt });
  ...
  const upcomingVisitSeeds = [
    {
      ...
      componentId: "component-102-ice-machine-water-pump",
    ...
    },
    {
      ...
      componentId: "component-105-dispenser-2-nozzle",
    ...
    },
    {
      ...
      assetId: "asset-104-roadside-sign-lighting",
      componentId: "component-104-roadside-sign-lighting-lighting-circuit",
  ...
  upcomingVisitSeeds.forEach((seed, index) => {
    ...
    workOrders.push({ id: workOrderId, organizationId: organization.id, number: seed.number, storeId: store.id, requestId, problem: seed.problem, authorizedScope: seed.scope, categoryKey: seed.categoryKey, taxonomyNodeId: taxonomyByCategory[seed.categoryKey], assetId: "assetId" in seed ? seed.assetId : undefined, componentId: "componentId" in seed ? seed.componentId : undefined, priority: "routine", status: "scheduled", accountableParty: vendor.name, nextAction: "Vendor arrives for the confirmed visit and checks in", dueAt: seed.startsAt, escalationTo: "Facilities coordinator", nte: { amountMinor: seed.nteAmountMinor, currency: "USD" }, createdAt });
    ...
    const asset = "assetId" in seed ? assets.find((candidate) => candidate.id === seed.assetId) : undefined;
    issuances.push({ id: issuanceId, organizationId: organization.id, workOrderId, assignmentId, revision: 1, immutablePayloadJson: JSON.stringify({ organizationName: organization.name, workOrderNumber: seed.number, store: { id: store.id, storeNumber: store.storeNumber, name: store.name, formattedAddress: [store.address1, `${store.city}, ${store.state} ${store.postalCode}`].join(", ") }, vendor: { id: vendor.id, name: vendor.name }, problem: seed.problem, priority: "routine", authorizedScope: seed.scope, categoryKey: seed.categoryKey, asset: asset ? { id: asset.id, name: asset.name, assetTag: asset.assetTag } : undefined, requestedTiming: seed.startsAt, nte: { amountMinor: seed.nteAmountMinor, currency: "USD" }, billingInstruction: `Reference operator work order ${seed.number} on all service paperwork and invoices.` }), channel: "email", issuedAt });
  ...
  const addProgramOccurrence = (input: {
    ...
    pmOccurrences.push({ id: `pm-occurrence-${input.plan.id.replace(/^pm-plan-/, "")}-${input.key}`, organizationId: organization.id, planId: input.plan.id, storeId: input.plan.storeId!, assetId: input.plan.assetId, programId: input.plan.programId, programVersion: input.plan.programVersion, planVersion: 1, dueAt: input.dueAt, windowStartsAt, windowEndsAt, status: input.status, completedAt: input.completedAt, result: input.completedAt ? "Manager-attested historical completion imported without platform work-order or visit evidence" : undefined, exceptionReason: input.exceptionReason, recurrenceKey: `${input.plan.id}:${input.key}`, createdAt: new Date(Date.parse(input.dueAt) - input.plan.cadenceDays * 86_400_000).toISOString() });
  };
  ...
  workOrders.push(
    { id: serviceRunReactiveWorkId, organizationId: organization.id, number: "CPS-2026-0118", storeId: "store-northline-104", problem: "Beer-cave door is icing along the lower hinge and needs an approved seal adjustment", authorizedScope: "Inspect the hinge and seal, correct alignment, and document any part requirement or additional recommended work.", categoryKey: "refrigeration", taxonomyNodeId: "taxonomy-northline-beer_caves", assetId: "asset-104-beer-cave", priority: "routine", status: "scheduled", version: 0, accountableParty: "ColdLine Refrigeration & HVAC", nextAction: "Arrive for the confirmed service appointment", dueAt: at(8, 30, 13, 30), escalationTo: "Facilities coordinator", nte: { amountMinor: 75_000, currency: "USD" }, createdAt: at(8, 10, 14) },
    { id: serviceRunPmWorkId, organizationId: organization.id, number: "CPS-2026-PM-105-5", storeId: "store-northline-105", problem: "Complete the quarterly refrigeration preventive-maintenance occurrence and asset checklist", authorizedScope: "Inspect, clean, measure, and document the Store 105 beer-cave refrigeration system under the approved PM Program.", categoryKey: "refrigeration", taxonomyNodeId: "taxonomy-northline-beer_caves", assetId: "asset-105-beer-cave", priority: "planned", status: "scheduled", version: 0, accountableParty: "ColdLine Refrigeration & HVAC", nextAction: "Arrive for the confirmed service appointment", dueAt: at(8, 30, 15, 20), escalationTo: "Facilities coordinator", nte: { amountMinor: 50_000, currency: "USD" }, createdAt: at(8, 10, 14, 5) },
  );
  assignments.push(
  ...
  [
    { workOrderId: serviceRunReactiveWorkId, assignmentId: "assignment-service-run-104", issuanceId: "issuance-service-run-104-r1", responseId: "response-service-run-104", appointmentId: "appointment-service-run-104", startsAt: at(8, 30, 13, 30) },
    { workOrderId: serviceRunPmWorkId, assignmentId: "assignment-service-run-105", issuanceId: "issuance-service-run-105-r1", responseId: "response-service-run-105", appointmentId: "appointment-service-run-105", startsAt: at(8, 30, 15, 20) },
  ].forEach((record, index) => {
    const workOrder = workOrders.find((candidate) => candidate.id === record.workOrderId)!;
    const store = stores.find((candidate) => candidate.id === workOrder.storeId)!;
    const issuedAt = at(8, 10, 15, 10 + index * 5);
    issuances.push({ id: record.issuanceId, organizationId: organization.id, workOrderId: workOrder.id, assignmentId: record.assignmentId, revision: 1, immutablePayloadJson: JSON.stringify({ organizationName: organization.name, workOrderNumber: workOrder.number, store: { id: store.id, storeNumber: store.storeNumber, name: store.name, formattedAddress: [store.address1, `${store.city}, ${store.state} ${store.postalCode}`].join(", ") }, vendor: { id: "vendor-northline-summit", name: "ColdLine Refrigeration & HVAC" }, problem: workOrder.problem, priority: workOrder.priority, authorizedScope: workOrder.authorizedScope, categoryKey: workOrder.categoryKey, requestedTiming: record.startsAt, nte: workOrder.nte, billingInstruction: `Reference operator work order ${workOrder.number} on any related service paperwork.` }), channel: "email", issuedAt });
    vendorResponses.push({ id: record.responseId, organizationId: organization.id, workOrderId: workOrder.id, assignmentId: record.assignmentId, issuanceId: record.issuanceId, response: "accepted", responderName: "ColdLine Refrigeration & HVAC dispatch", message: "The requested store appointment is confirmed.", respondedAt: new Date(Date.parse(issuedAt) + 30 * 60_000).toISOString() });
    serviceAppointments.push({ id: record.appointmentId, organizationId: organization.id, workOrderId: workOrder.id, assignmentId: record.assignmentId, issuanceId: record.issuanceId, sourceVendorResponseId: record.responseId, status: "confirmed", proposedBy: "vendor", startsAt: record.startsAt, note: "Vendor-confirmed service appointment", createdByMembershipId: "membership-northline-facilities", createdAt: new Date(Date.parse(issuedAt) + 35 * 60_000).toISOString() });
  });
...
export function assertOpsFixture(fixture: OpsFixture) {
  ...
  fixture.visits.forEach((row) => {
    ...
    if (row.outcome && links.length === 1 && links[0]?.outcome !== siteVisitOutcomeFromLegacy(row.outcome)) throw new Error(`Visit ${row.id} scalar outcome does not match its work-order outcome`);
