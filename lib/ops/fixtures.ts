...
function buildFixture(): OpsFixture {
  ...
  const recentServiceStories: RecentServiceStory[] = [
    ...
    { key: "aug-111-plumbing", storeNumber: "111", categoryKey: "plumbing", problem: "Restroom hand-sink supply connection is leaking into the cabinet", providerKind: "outside_vendor", vendorId: "vendor-northline-cedar", technicianName: "Imani Lewis", providerName: "Cedar Mechanical", priority: "routine", submittedAt: at(8, 9, 16, 45), checkedInAt: at(8, 10, 12, 15), durationMinutes: 82, startedChannel: "secure_link", endedChannel: "store_device", locationResult: "verified", outcome: "resolved", outcomeNotes: "Replaced the supply stop and flex connector, dried the cabinet, and verified no leakage under repeated use.", workStatus: "completed_pending_review", nteAmountMinor: 125_000, laborAmountMinor: 37_500, materialsAmountMinor: 18_900, followUpAction: "Review the store confirmation and recorded cost, then close the work order", followUpDueAt: at(8, 11, 17) },
  ];
  ...
  requests.push(
    ...
    { id: "request-current-103-gfci-reset", organizationId: organization.id, reference: "REQ-26-103C", storeId: "store-northline-103", reporterName: "Riley Adams", reporterEmployeeId: "E6103", problem: "Coffee counter receptacles lost power during cleaning", priority: "routine", status: "closed", submittedAt: at(8, 9, 10, 15) },
  );
  ...
  requests.push({ id: declinedRequestId, organizationId: organization.id, reference: "REQ-26-114C", storeId: "store-northline-114", reporterName: "Jamie Cole", reporterEmployeeId: "E6214", problem: declinedProblem, priority: "urgent", status: "converted", submittedAt: at(8, 9, 20, 40), convertedWorkOrderId: declinedWorkOrderId });
  workOrders.push({ id: declinedWorkOrderId, organizationId: organization.id, number: "NL-2026-0212", storeId: "store-northline-114", requestId: declinedRequestId, problem: declinedProblem, authorizedScope: "Diagnose the west-island canopy circuit and restore safe lighting within the authorization limit.", categoryKey: "electrical", taxonomyNodeId: "taxonomy-northline-canopy_lighting", priority: "urgent", status: "waiting_on_vendor", accountableParty: "Facilities coordinator", nextAction: "Select a backup electrical vendor after BrightPath declined", dueAt: at(8, 11, 12), escalationTo: "Facilities director", nte: { amountMinor: 180_000, currency: "USD" }, createdAt: at(8, 9, 21) });
  assignments.push(
    { id: declinedAssignmentId, organizationId: organization.id, workOrderId: declinedWorkOrderId, kind: "outside_vendor", vendorId: "vendor-northline-brightpath", status: "declined", assignedAt: at(8, 9, 21, 10) },
    { id: "assignment-current-114-canopy-choose-later", organizationId: organization.id, workOrderId: declinedWorkOrderId, kind: "choose_later", status: "pending", assignedAt: at(8, 10, 8, 25), supersedesAssignmentId: declinedAssignmentId },
  );
  issuances.push({ id: declinedIssuanceId, organizationId: organization.id, workOrderId: declinedWorkOrderId, assignmentId: declinedAssignmentId, revision: 1, immutablePayloadJson: JSON.stringify({ organizationName: organization.name, workOrderNumber: "NL-2026-0212", store: { id: "store-northline-114", storeNumber: "114", name: "Northline North Market", formattedAddress: "1414 North Market Street, North Market, OH 43540" }, vendor: { id: "vendor-northline-brightpath", name: "BrightPath Electrical" }, problem: declinedProblem, priority: "urgent", authorizedScope: "Diagnose the west-island canopy circuit and restore safe lighting within the authorization limit.", categoryKey: "electrical", requestedTiming: at(8, 10, 16), nte: { amountMinor: 180_000, currency: "USD" }, billingInstruction: "Reference operator work order NL-2026-0212 on all service paperwork and invoices." }), channel: "email", issuedAt: at(8, 9, 21, 20) });
  vendorResponses.push({ id: "response-current-114-declined", organizationId: organization.id, workOrderId: declinedWorkOrderId, assignmentId: declinedAssignmentId, issuanceId: declinedIssuanceId, response: "declined", responderName: "BrightPath Electrical dispatch", message: "No lift-qualified technician is available within the requested service window.", respondedAt: at(8, 10, 8, 15) });
  ...
  invoiceReferences.push({ id: "invoice-summit-104-compressor", organizationId: organization.id, vendorId: "vendor-northline-summit", invoiceNumber: "SUM-104-2607", invoiceDate: "2026-07-11", grossAmount: { amountMinor: 1_157_500, currency: "USD" }, operatorWorkOrderNumber: "NL-2026-0104", matchStatus: "confirmed", createdAt: at(7, 11, 14) });
  ...
  invoiceReferences.push({ id: "invoice-summit-104-warranty-callback", organizationId: organization.id, vendorId: "vendor-northline-summit", invoiceNumber: "SUM-104-2611-W", invoiceDate: "2026-08-11", grossAmount: { amountMinor: 890_000, currency: "USD" }, operatorWorkOrderNumber: "NL-2026-0119", matchStatus: "confirmed", createdAt: at(8, 11, 14) });
  ...
  billedTripInvoice.exceptionReason = "The source invoice lists four mobilizations while two exact Visit / Work Order links are recorded. A reviewer must reconcile the difference with Summit.";
  ...
  invoiceExceptions.push({ id: "invoice-exception-history-104-4-duplicate", organizationId: organization.id, invoiceId: "invoice-history-104-4-duplicate", invoiceLineId: "invoice-line-history-104-4-duplicate", kind: "duplicate_invoice", status: "open", summary: "Exact source-document fingerprint match to SUMMIT-202603-104-4. The second record is held for human review and is not included in the spend projection.", amount: { amountMinor: 872_500, currency: "USD" }, detectedAt: at(3, 20, 14, 1) });
  valueEvents.push(
  ...
  const notificationRules: NonNullable<OpsFixture["notificationRules"]> = [
    ...
    { id: "notification-rule-vendor-commitment-region", organizationId: organization.id, eventKey: "vendor_commitment_received", emailEnabled: false, recipientRole: "regional_manager", updatedByMembershipId: "membership-northline-facilities", createdAt: at(8, 1, 8), updatedAt: at(8, 1, 8) },
