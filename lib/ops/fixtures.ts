...
function buildFixture(): OpsFixture {
  ...
  const workOrderVisitHolds: NonNullable<OpsFixture["workOrderVisitHolds"]> = heldWorkSeed.map(({ workOrder, posture, threshold }, index) => {
    ...
    workflowTasks.push({ id: `workflow-task-${workOrder.id}-hold`, organizationId: organization.id, workOrderId: workOrder.id, taskType: "choose_service_provider", title: "Wait for a matching vendor visit", reason: "A manager approved this low-priority item for opportunistic pickup by an approved vendor.", assigneeType: "role", assigneeRole: "facilities_admin", assigneeName: "Facilities coordinator", priority: "low", status: "open", blocking: true, requiredForProgress: true, dueAt: workOrder.dueAt!, applicableSlaClock: "scheduling", completionCriteria: "A matching vendor claims the work onsite, or facilities releases the hold.", escalationDestination: "Facilities director", escalationLevel: 0, createdByActorType: "user", createdByActorId: "membership-northline-facilities", createdByActorName: "Jordan Lee", createdAt: workOrder.createdAt });
