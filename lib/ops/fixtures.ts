...
function buildFixture(): OpsFixture {
  ...
  const return109IssuanceId = "issuance-northline-109-return-r1";
...
export function assertOpsFixture(fixture: OpsFixture) {
  ...
  fixture.vendorResponses.forEach((row) => { const issuance = fixture.issuances.find((item) => item.id === row.issuanceId); if (!issuanceIds.has(row.issuanceId) || issuance?.assignmentId !== row.assignmentId) throw new Error(`Vendor response ${row.id} has no issuance`); if (Date.parse(row.respondedAt) < Date.parse(issuance.issuedAt)) throw new Error(`Vendor response ${row.id} predates issuance`); });
