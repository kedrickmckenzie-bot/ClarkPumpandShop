...
function page<T extends { id: string }>(rows: T[], request: PageRequest = {}): Page<T> {
  ...
  const start = request.offset !== undefined
    ? Math.max(0, Math.floor(request.offset))
    : request.cursor
      ? Math.max(0, rows.findIndex((row) => row.id === request.cursor) + 1)
      : 0;
  const items = rows.slice(start, start + limit);
  return { items, nextCursor: start + limit < rows.length ? items.at(-1)?.id : undefined, totalCount: rows.length };
}
...
function mapTable(fixture: OpsFixture, table: string): Array<Record<string, unknown>> {
  ...
  if (!fixture[key]) (fixture as unknown as Record<string, unknown>)[key] = [];
...
function hydrateInserted(table: string, raw: Record<string, unknown>) {
  ...
  if (table === "ops_work_order_visit_holds") { if (row.internalReviewThresholdMinor !== undefined && row.internalReviewThresholdMinor !== null) row.internalReviewThreshold = { amountMinor: row.internalReviewThresholdMinor, currency: row.currency ?? "USD" }; delete row.internalReviewThresholdMinor; delete row.currency; }
...
class FixtureOpsRepository implements MutableOpsFixtureRepository {
  ...
  async getDashboardContext(scope: OrganizationScope) {
    return dashboardContextFromFixture(this.fixture, scope);
  }
  ...
  async listStoreIdsForMembership(organizationId: OpsId, membershipId: OpsId) {
    const grants = (await this.listScopeGrantsForMembership(organizationId, membershipId)).filter(grant => ["ops:*", "ops:read", "ops:write", "ops:read_write", "ops:store_manage", "ops:finance_read"].includes(grant.permission));
    return this.fixture.stores.filter(store => store.organizationId === organizationId && grants.some(grant =>
      grant.scopeKind === "organization" && grant.scopeId === organizationId || grant.scopeKind === "division" && grant.scopeId === store.divisionId || grant.scopeKind === "region" && grant.scopeId === store.regionId || grant.scopeKind === "store" && grant.scopeId === store.id
  ...
  async listAssetsForStore(organizationId: OpsId, storeId: OpsId) { return clone(this.fixture.assets.filter(a => a.organizationId === organizationId && a.storeId === storeId).sort((a,b) => a.name.localeCompare(b.name) || a.id.localeCompare(b.id))); }
  ...
  async listVendorComplianceAlerts(organizationId: OpsId, vendorId: OpsId) { return clone((this.fixture.vendorComplianceAlerts ?? []).filter((row) => row.organizationId === organizationId && row.vendorId === vendorId).sort((a, b) => b.createdAt.localeCompare(a.createdAt) || b.id.localeCompare(a.id))); }
  ...
  async getActionableVendorResponse(organizationId: OpsId, workOrderId: OpsId, assignmentId?: OpsId, issuanceId?: OpsId) { return clone(this.fixture.vendorResponses.filter(r => r.organizationId === organizationId && r.workOrderId === workOrderId && !(this.fixture.vendorContinuations ?? []).some(c => c.organizationId === organizationId && c.workOrderId === workOrderId && c.vendorResponseId === r.id) && (r.response === "declined" || ["proposed_date","question"].includes(r.response) && r.assignmentId === assignmentId && r.issuanceId === issuanceId)).sort((a,b) => Date.parse(b.respondedAt)-Date.parse(a.respondedAt) || (a.id < b.id ? 1 : -1))[0] ?? null); }
  ...
  async getServiceAppointment(organizationId: OpsId, appointmentId: OpsId): Promise<ServiceAppointment | null> { return clone((this.fixture.serviceAppointments ?? []).find((row) => row.organizationId === organizationId && row.id === appointmentId) ?? null); }
  ...
  async listAllVendorComplianceDocumentsForWorker() { return clone(this.fixture.vendorComplianceDocuments.slice().sort((a, b) => a.organizationId.localeCompare(b.organizationId) || a.vendorId.localeCompare(b.vendorId) || a.documentType.localeCompare(b.documentType) || b.createdAt.localeCompare(a.createdAt))); }
  ...
  async putSavedView(input: { organizationId: OpsId; id: OpsId; ownerMembershipId: OpsId; surface: string; name: string; queryString: string; createdAt: string }): Promise<void> { const candidate = clone(this.fixture); candidate.savedViews = (candidate.savedViews ?? []).filter((row) => !(row.organizationId === input.organizationId && row.ownerMembershipId === input.ownerMembershipId && row.surface === input.surface && row.name === input.name)); candidate.savedViews.push(clone(input)); this.fixture = candidate; }
