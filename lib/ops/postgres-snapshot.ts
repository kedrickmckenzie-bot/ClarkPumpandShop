...
export async function loadOpsFixtureSnapshotFromPostgres(
  ...
  return {
    ...
    workOrderVisitHolds: workOrderVisitHolds.map((row) => ({ ...row, version: Number(row.version ?? 0), internalReviewThreshold: row.internalReviewThresholdMinor == null ? undefined : { amountMinor: Number(row.internalReviewThresholdMinor), currency: row.currency ?? "USD" }, claimedVisitId: optional(row.claimedVisitId), claimedVendorId: optional(row.claimedVendorId), claimedAt: optional(row.claimedAt), createdByMembershipId: optional(row.createdByMembershipId) })) as unknown as OpsFixture["workOrderVisitHolds"],
