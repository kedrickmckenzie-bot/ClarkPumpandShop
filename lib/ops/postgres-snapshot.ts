...
async function tenantRows(pool: PostgresPoolLike, table: string, organizationId: OpsId, includedTables?: ReadonlySet<string>) {
  if (includedTables && !includedTables.has(table)) return [];
...
export async function loadOpsFixtureSnapshotFromPostgres(
  ...
  const [
    ...
    rows("ops_divisions"),
    rows("ops_regions"),
    rows("ops_taxonomy_nodes"),
    rows("ops_equipment_templates"),
    rows("ops_component_templates"),
    rows("ops_stores"),
    rows("ops_memberships"),
    rows("ops_scope_grants"),
    rows("ops_vendors"),
    rows("ops_vendor_reminders"),
    rows("ops_vendor_specialties"),
    rows("ops_vendor_coverage"),
    rows("ops_vendor_qualifications"),
    rows("ops_vendor_compliance_documents"),
    rows("ops_vendor_compliance_alerts"),
    rows("ops_vendor_contracts"),
    rows("ops_contract_versions"),
    rows("ops_contract_scopes"),
    rows("ops_rate_card_lines"),
    rows("ops_service_level_policies"),
    rows("ops_scheduling_policies"),
    rows("ops_vendor_capacity"),
    rows("ops_requests"),
    rows("ops_request_impact_assessments"),
    rows("ops_work_orders"),
    rows("ops_work_order_visit_holds"),
    rows("ops_approval_policies"),
    rows("ops_approval_requests"),
    rows("ops_approval_decisions"),
    rows("ops_work_order_assignments"),
    rows("ops_work_order_issuances"),
    rows("ops_vendor_responses"),
    rows("ops_work_order_estimate_requests"),
    rows("ops_vendor_estimate_proposals"),
    rows("ops_work_prices"),
    rows("ops_visit_sessions"),
    rows("ops_site_visit_work_orders"),
    rows("ops_work_order_verifications"),
    rows("ops_visit_evidence"),
    rows("ops_files"),
    rows("ops_entity_files"),
    rows("ops_follow_ups"),
    rows("ops_workflow_tasks"),
    rows("ops_workflow_task_sla_pauses"),
    rows("ops_workflow_task_sla_resumes"),
    rows("ops_exceptions"),
    rows("ops_replacement_profiles"),
    rows("ops_assets"),
    rows("ops_replacement_benchmarks"),
    rows("ops_asset_replacement_overrides"),
    rows("ops_replacement_events"),
    rows("ops_lifecycle_recommendations"),
    rows("ops_asset_components"),
    rows("ops_component_lifecycle_events"),
    rows("ops_maintenance_programs"),
    rows("ops_checklist_templates"),
    rows("ops_pm_plans"),
    rows("ops_pm_occurrences"),
    rows("ops_pm_work_items"),
    rows("ops_checklist_responses"),
    rows("ops_service_runs"),
    rows("ops_route_stops"),
    rows("ops_service_run_work_orders"),
    rows("ops_service_run_responses"),
    rows("ops_vendor_warranty_profiles"),
    rows("ops_warranty_rules"),
    rows("ops_warranty_coverage_lines"),
    rows("ops_repair_items"),
    rows("ops_applied_warranties"),
    rows("ops_warranty_amendments"),
    rows("ops_manufacturer_warranties"),
    rows("ops_warranty_cases"),
    rows("ops_quotes"),
    rows("ops_authorizations"),
    rows("ops_invoices"),
    rows("ops_invoice_lines"),
    rows("ops_accounting_invoice_sources"),
    rows("ops_invoice_line_allocations"),
    rows("ops_invoice_exceptions"),
    rows("ops_invoice_adjustments"),
    rows("ops_service_discrepancies"),
    rows("ops_value_events"),
    rows("ops_cost_lines"),
  ...
  return {
    asOf,
    workPrices: workPrices.map(workPriceFrom),
    ...
    vendorReminders: vendorReminders.map((row) => ({ ...row, note: optional(row.note), createdByActorId: optional(row.createdByActorId), completedByActorType: optional(row.completedByActorType), completedByActorId: optional(row.completedByActorId), completedByActorName: optional(row.completedByActorName), completedAt: optional(row.completedAt), completionNote: optional(row.completionNote) })) as unknown as OpsFixture["vendorReminders"],
    ...
    vendorComplianceAlerts: vendorComplianceAlerts.map((row) => ({ ...row, reminderId: optional(row.reminderId) })) as unknown as OpsFixture["vendorComplianceAlerts"],
    ...
    requests: requests.map((row) => ({ ...row, version: Number(row.version ?? 0), reporterEmployeeId: optional(row.reporterEmployeeId), acknowledgedAt: optional(row.acknowledgedAt), acknowledgedByActorType: optional(row.acknowledgedByActorType), acknowledgedByActorId: optional(row.acknowledgedByActorId), acknowledgedByActorName: optional(row.acknowledgedByActorName), linkedWorkOrderId: optional(row.linkedWorkOrderId), linkedAt: optional(row.linkedAt), linkedByActorType: optional(row.linkedByActorType), linkedByActorId: optional(row.linkedByActorId), linkedByActorName: optional(row.linkedByActorName), convertedWorkOrderId: optional(row.convertedWorkOrderId) })) as unknown as OpsFixture["requests"],
    ...
    workOrders: workOrders.map((row) => ({ ...row, internalReviewThresholdMinor: row.internalReviewThresholdMinor == null ? undefined : Number(row.internalReviewThresholdMinor), internalReviewCurrency: optional(row.internalReviewCurrency), version: Number(row.version ?? 0), requestId: optional(row.requestId), authorizedScope: optional(row.authorizedScope), categoryKey: optional(row.categoryKey), taxonomyNodeId: optional(row.taxonomyNodeId), assetId: optional(row.assetId), componentId: optional(row.componentId), internalAccountableType: optional(row.internalAccountableType), internalAccountableId: optional(row.internalAccountableId), dueAt: optional(row.dueAt), escalationTo: optional(row.escalationTo), nte: row.nteAmountMinor == null ? undefined : { amountMinor: Number(row.nteAmountMinor), currency: row.nteCurrency ?? "USD" }, repairEstimate: row.repairEstimateAmountMinor == null ? undefined : { amountMinor: Number(row.repairEstimateAmountMinor), currency: row.repairEstimateCurrency ?? "USD" }, estimatedServiceExtensionMonths: optional(row.estimatedServiceExtensionMonths), vendorServiceTicketNumber: optional(row.vendorServiceTicketNumber), vendorInvoiceNumber: optional(row.vendorInvoiceNumber), externalAccountingPo: optional(row.externalAccountingPo), resolvedAt: optional(row.resolvedAt), closedAt: optional(row.closedAt) })) as unknown as OpsFixture["workOrders"],
    workOrderVisitHolds: workOrderVisitHolds.map((row) => ({ ...row, version: Number(row.version ?? 0), internalReviewThreshold: row.internalReviewThresholdMinor == null ? undefined : { amountMinor: Number(row.internalReviewThresholdMinor), currency: row.currency ?? "USD" }, claimedVisitId: optional(row.claimedVisitId), claimedVendorId: optional(row.claimedVendorId), claimedAt: optional(row.claimedAt), plannedReviewAppointmentId: optional(row.plannedReviewAppointmentId), plannedReviewSelectedAt: optional(row.plannedReviewSelectedAt), plannedReviewSelectedByMembershipId: optional(row.plannedReviewSelectedByMembershipId), createdByMembershipId: optional(row.createdByMembershipId) })) as unknown as OpsFixture["workOrderVisitHolds"],
    ...
    siteVisitWorkOrders: siteVisitWorkOrders.map((row) => ({ ...row, ordinal: Number(row.ordinal), linkedByActorId: optional(row.linkedByActorId), workOrderHoldId: optional(row.workOrderHoldId), outcome: optional(row.outcome), outcomeNotes: optional(row.outcomeNotes), outcomeRecordedByActorType: optional(row.outcomeRecordedByActorType), outcomeRecordedByActorId: optional(row.outcomeRecordedByActorId), outcomeRecordedByActorName: optional(row.outcomeRecordedByActorName), outcomeRecordedAt: optional(row.outcomeRecordedAt), followUpId: optional(row.followUpId), vendorFollowUpTiming: optional(row.vendorFollowUpTiming) })) as unknown as OpsFixture["siteVisitWorkOrders"],
    workOrderVerifications: workOrderVerifications.map((row) => ({ ...row, cycle: Number(row.cycle), basis: optional(row.basis), verificationScope: optional(row.verificationScope), reason: optional(row.reason) })) as unknown as OpsFixture["workOrderVerifications"],
    ...
    components: components.map((row) => ({ ...row, parentComponentId: optional(row.parentComponentId), partNumber: optional(row.partNumber), serialNumber: optional(row.serialNumber), installedAt: optional(row.installedAt), warrantyEndsAt: optional(row.warrantyEndsAt), removedAt: optional(row.removedAt), replacedByComponentId: optional(row.replacedByComponentId) })) as unknown as OpsFixture["components"],
    componentLifecycleEvents: componentLifecycleEvents.map((row) => ({ ...row, serialNumber: optional(row.serialNumber), rootCause: optional(row.rootCause), laborCost: { amountMinor: Number(row.laborCostMinor), currency: row.currency }, partCost: { amountMinor: Number(row.partCostMinor), currency: row.currency }, expectedLifeMonths: optional(row.expectedLifeMonths), warrantyEndsAt: optional(row.warrantyEndsAt) })) as unknown as OpsFixture["componentLifecycleEvents"],
    ...
    accountingInvoiceSources: accountingInvoiceSources as unknown as OpsFixture["accountingInvoiceSources"],
