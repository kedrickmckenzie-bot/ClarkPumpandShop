...
import type { PmAnalysisQuery } from "./pm-analysis-query";
import type { PmScheduleQuery } from "./pm-schedule-query";
...
class SqlOpsRepository implements OpsRepository {
  ...
  async listWorkPrices(organizationId: string, query: WorkPriceQuery) {
    const clauses = ["organization_id = ?"]; const params: unknown[] = [organizationId];
    for (const [field, value] of [["work_order_id", query.workOrderId], ["asset_id", query.assetId], ["profile_id", query.profileId], ["kind", query.kind]]) {
      if (value) { clauses.push(field + " = ?"); params.push(value); }
    }
    if (query.storeIds !== undefined) {
      if (!query.storeIds.length) return { items: [], total: 0 };
      clauses.push("store_id IN (" + query.storeIds.map(() => "?").join(", ") + ")"); params.push(...query.storeIds);
    }
    const where = clauses.join(" AND ");
    const limit = Math.max(1, Math.min(50, Math.floor(query.limit ?? 20)));
    const offset = Math.max(0, Math.floor(query.offset ?? 0));
    const [rows, count] = await Promise.all([
      this.all("SELECT * FROM ops_work_prices WHERE " + where + " ORDER BY recorded_at DESC, id DESC LIMIT ? OFFSET ?", [...params, limit, offset]),
      this.first("SELECT COUNT(*) AS total FROM ops_work_prices WHERE " + where, params),
    ]);
    return { items: rows.map(workPriceFrom), total: Number(count?.total ?? 0) };
  }
  constructor(
    private readonly driver: OpsSqlDriver,
    readonly kind: "d1" | "postgres" = "d1",
  ) {}

  private async first(sql: string, params: readonly unknown[] = []) { return (await this.driver.query({ sql, params })).rows[0] ?? null; }
  private async all(sql: string, params: readonly unknown[] = []) { return (await this.driver.query({ sql, params })).rows; }
  ...
  async listStoreIdsForMembership(organizationId: OpsId, membershipId: OpsId) {
    return (await this.all("SELECT s.id FROM ops_stores s WHERE s.organization_id = ? AND EXISTS (SELECT 1 FROM ops_scope_grants g WHERE g.organization_id = s.organization_id AND g.membership_id = ? AND g.permission IN ('ops:*', 'ops:read', 'ops:write', 'ops:read_write', 'ops:store_manage', 'ops:finance_read') AND ((g.scope_kind = 'organization' AND g.scope_id = s.organization_id) OR (g.scope_kind = 'division' AND g.scope_id = s.division_id) OR (g.scope_kind = 'region' AND g.scope_id = s.region_id) OR (g.scope_kind = 'store' AND g.scope_id = s.id))) ORDER BY s.id", [organizationId, membershipId])).map(row => text(row, "id"));
  }
  ...
  async getAsset(organizationId: OpsId, assetId: OpsId) { const row = await this.first("SELECT * FROM ops_assets WHERE organization_id = ? AND id = ?", [organizationId, assetId]); return row ? assetFrom(row) : null; }
  ...
  async listAssetsForStore(organizationId: OpsId, storeId: OpsId): Promise<Asset[]> { return (await this.all("SELECT * FROM ops_assets WHERE organization_id = ? AND store_id = ? ORDER BY name, id", [organizationId, storeId])).map(assetFrom); }
  ...
  async readVendorOnboardingSelection(organizationId: OpsId, scopeIds: string[], specialtyKeys: string[]) { return (await import("./vendor-onboarding-query")).queryVendorOnboarding(this.driver,organizationId,scopeIds,specialtyKeys); }
  async listWarrantyQueue(scope: OrganizationScope, query: import("./warranty-queue-query").WarrantyQueueQuery) { return (await import("./warranty-queue-sql")).queryWarrantyQueue(this.driver,scope,query); }
  async readOnboardingConfiguration(organizationId: OpsId) { const [regions,specialties]=await Promise.all([this.all("SELECT * FROM ops_regions WHERE organization_id=? ORDER BY id",[organizationId]),this.all("SELECT canonical_key,MIN(display_name) AS display_name FROM ops_vendor_specialties WHERE organization_id=? GROUP BY canonical_key ORDER BY canonical_key",[organizationId])]);return {regions:regions.map(r=>({id:text(r,"id"),organizationId,divisionId:maybeText(r,"division_id"),code:text(r,"code"),name:text(r,"name"),createdAt:text(r,"created_at")})),vendorSpecialties:specialties.map(r=>({organizationId,canonicalKey:text(r,"canonical_key"),displayName:text(r,"display_name")}))}; }
  async listInvoiceEvidence(scope: OrganizationScope, query: import("./invoice-evidence-query").InvoiceEvidenceQuery) { return (await import("./invoice-evidence-sql")).queryInvoiceEvidence(this.driver, scope, query); }
  ...
  async getActionableVendorResponse(organizationId: OpsId, workOrderId: OpsId, assignmentId?: OpsId, issuanceId?: OpsId) { const row = await this.first(`SELECT r.* FROM ops_vendor_responses r WHERE r.organization_id=? AND r.work_order_id=? AND NOT EXISTS (SELECT 1 FROM ops_vendor_continuations c WHERE c.organization_id=r.organization_id AND c.work_order_id=r.work_order_id AND c.vendor_response_id=r.id) AND (r.response='declined' OR (r.response IN ('proposed_date','question') AND r.assignment_id=? AND r.issuance_id=?)) ORDER BY ${this.driver.dialect === "postgres" ? "r.responded_at" : "julianday(r.responded_at)"} DESC,r.id ${this.driver.dialect === "postgres" ? 'COLLATE "C"' : "COLLATE BINARY"} DESC LIMIT 1`, [organizationId,workOrderId,assignmentId??null,issuanceId??null]); return row ? vendorResponseFrom(row) : null; }
  ...
  private async workOrderRows(scope: OrganizationScope, query: WorkOrderListQuery & { workOrderId?: OpsId; assetId?: OpsId; unbounded?: boolean } = {}) {
    ...
    if (query.categoryPath?.length) {
      const pathConditions = query.categoryPath.map((segment, index) => {
        ...
        const groupIncludesCategory = `lower(${sqlJsonArrayText(this.driver.dialect, "wa.group_path_json", 0)}) = ${category}`;
        const fallback = index === 0 ? category : sqlJsonArrayText(this.driver.dialect, "wa.group_path_json", index - 1);
        const value = `CASE WHEN ${groupIncludesCategory} THEN ${sqlJsonArrayText(this.driver.dialect, "wa.group_path_json", index)} ELSE ${fallback} END`;
  ...
  async listVendors(scope: OrganizationScope, search = "", request: PageRequest = {}) {
    ...
    const rows = await this.all(`WITH scoped_stores AS (
      ...
      SELECT v.*,
      (SELECT ${sqlStringAggregate(this.driver.dialect, "display_name")} FROM ops_vendor_specialties sp WHERE sp.organization_id = v.organization_id AND sp.vendor_id = v.id) AS specialties,
  ...
  async getAssetWarrantySources(organizationId: OpsId, assetId: OpsId) {
    ...
    return {
      ...
      manufacturerWarranties: manufacturer.map((r): import("./types").ManufacturerWarranty => ({ id:text(r,"id"), organizationId, assetId, componentId:maybeText(r,"component_id"), providerKind: (maybeText(r,"provider_kind") ?? "manufacturer") as "manufacturer" | "vendor", vendorId:maybeText(r,"vendor_id"), workOrderId:maybeText(r,"work_order_id"), title:maybeText(r,"title"), manufacturer:text(r,"manufacturer"), partsCoverage:text(r,"parts_coverage"), laborCoverage:text(r,"labor_coverage"), startDate:text(r,"start_date"), expirationDate:text(r,"expiration_date"), authorizedProviderRule:maybeText(r,"authorized_provider_rule"), claimRequirements:maybeText(r,"claim_requirements"), createdAt:text(r,"created_at") })),
    };
  }
  ...
  async claimOutboxMessage(organizationId: OpsId, id: OpsId, claimedAt: string): Promise<boolean> {
    const result = await this.driver.query({ sql: "UPDATE ops_outbox_messages SET status = ?, claimed_at = ?, attempt_count = attempt_count + 1 WHERE organization_id = ? AND id = ? AND status = ? RETURNING id", params: ["processing", claimedAt, organizationId, id, "pending"] });
    return result.affectedRows > 0 || result.rows.length > 0;
  }
  ...
  async tryBeginJobRun(input: { organizationId: OpsId; jobRunId: OpsId; jobType: string; slotKey: string; startedAt: string }): Promise<boolean> {
    const result = await this.driver.query({ sql: "INSERT INTO ops_job_runs (id, organization_id, job_type, slot_key, status, started_at, processed_count, failed_count, created_at) VALUES (?, ?, ?, ?, ?, ?, 0, 0, ?) ON CONFLICT DO NOTHING RETURNING id", params: [input.jobRunId, input.organizationId, input.jobType, input.slotKey, "running", input.startedAt, input.startedAt] });
    return result.affectedRows > 0 || result.rows.length > 0;
  }
  ...
  async deleteSavedView(organizationId: OpsId, ownerMembershipId: OpsId, id: OpsId): Promise<boolean> { const result = await this.driver.query({ sql: "DELETE FROM ops_saved_views WHERE organization_id = ? AND owner_membership_id = ? AND id = ? RETURNING id", params: [organizationId, ownerMembershipId, id] }); return result.affectedRows > 0 || result.rows.length > 0; }
...
export function createOpsSqlRepository(
  ...
  return new SqlOpsRepository(driver, kind);
}
