...
export async function loadOpsFixtureSnapshotFromD1(
  ...
  return {
    ...
    workOrderVisitHolds: workOrderVisitHoldRows.map((row) => ({ ...row, internalReviewThreshold: row.internalReviewThresholdMinor == null ? undefined : { amountMinor: row.internalReviewThresholdMinor, currency: row.currency ?? "USD" }, claimedVisitId: optional(row.claimedVisitId), claimedVendorId: optional(row.claimedVendorId), claimedAt: optional(row.claimedAt), createdByMembershipId: optional(row.createdByMembershipId) })) as OpsFixture["workOrderVisitHolds"],
