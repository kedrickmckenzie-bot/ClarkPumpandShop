    ...
    for (const plan of orgPlans) {
      try {
        ...
        await repository.atomicWrite([
          insertStatement("ops_pm_occurrences", { id: occurrenceId, organization_id: organizationId, plan_id: plan.id, store_id: last.storeId, asset_id: last.assetId ?? null, program_id: plan.programId ?? last.programId ?? null, program_version: plan.programVersion ?? last.programVersion ?? null, plan_version: last.planVersion ?? null, due_at: candidateDueAt, window_starts_at: candidateDueAt, window_ends_at: windowEndsAt, status: "upcoming", recurrence_key: recurrenceKey, created_at: occurredAt }),
