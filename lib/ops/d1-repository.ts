...
function siteVisitWorkOrderFrom(row: Row): SiteVisitWorkOrder { return { id: text(row, "id"), organizationId: text(row, "organization_id"), visitId: text(row, "visit_id"), workOrderId: text(row, "work_order_id"), ordinal: Number(row.ordinal), linkedByActorType: text(row, "linked_by_actor_type") as SiteVisitWorkOrder["linkedByActorType"], linkedByActorId: maybeText(row, "linked_by_actor_id"), linkedByActorName: text(row, "linked_by_actor_name"), linkedAt: text(row, "linked_at"), outcome: maybeText(row, "outcome") as SiteVisitWorkOrder["outcome"], outcomeNotes: maybeText(row, "outcome_notes"), outcomeRecordedByActorType: maybeText(row, "outcome_recorded_by_actor_type") as SiteVisitWorkOrder["outcomeRecordedByActorType"], outcomeRecordedByActorId: maybeText(row, "outcome_recorded_by_actor_id"), outcomeRecordedByActorName: maybeText(row, "outcome_recorded_by_actor_name"), outcomeRecordedAt: maybeText(row, "outcome_recorded_at"), followUpId: maybeText(row, "follow_up_id") }; }
function workOrderVerificationFrom(row: Row): WorkOrderVerification { return { id: text(row, "id"), organizationId: text(row, "organization_id"), workOrderId: text(row, "work_order_id"), siteVisitWorkOrderId: text(row, "site_visit_work_order_id"), outcome: text(row, "outcome") as WorkOrderVerification["outcome"], outcomeRecordedAt: text(row, "outcome_recorded_at"), cycle: Number(row.cycle), decision: text(row, "decision") as WorkOrderVerification["decision"], reason: maybeText(row, "reason"), decidedByMembershipId: text(row, "decided_by_membership_id"), decidedByName: text(row, "decided_by_name"), decidedAt: text(row, "decided_at") }; }
...
class D1OpsRepository implements OpsRepository {
  ...
  async getRequest(organizationId: OpsId, requestId: OpsId) { const row = await this.first("SELECT * FROM ops_requests WHERE organization_id = ? AND id = ?", [organizationId, requestId]); return row ? { id: text(row, "id"), organizationId: text(row, "organization_id"), reference: text(row, "reference"), storeId: text(row, "store_id"), reporterName: text(row, "reporter_name"), reporterEmployeeId: maybeText(row, "reporter_employee_id"), problem: text(row, "problem"), priority: text(row, "priority") as ServiceRequest["priority"], status: text(row, "status") as ServiceRequest["status"], version: Number(row.version ?? 0), submittedAt: text(row, "submitted_at"), convertedWorkOrderId: maybeText(row, "converted_work_order_id") } : null; }
