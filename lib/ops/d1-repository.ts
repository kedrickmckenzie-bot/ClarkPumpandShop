...
function workOrderFrom(row: Row): WorkOrder {
  ...
  return { id: text(row, "id"), organizationId: text(row, "organization_id"), number: text(row, "number"), storeId: text(row, "store_id"), requestId: maybeText(row, "request_id"), problem: text(row, "problem"), authorizedScope: maybeText(row, "authorized_scope"), categoryKey: maybeText(row, "category_key"), taxonomyNodeId: maybeText(row, "taxonomy_node_id"), assetId: maybeText(row, "asset_id"), componentId: maybeText(row, "component_id"), priority: text(row, "priority") as WorkOrder["priority"], status: text(row, "status") as WorkOrder["status"], accountableParty: text(row, "accountable_party"), nextAction: text(row, "next_action"), dueAt: maybeText(row, "due_at"), escalationTo: maybeText(row, "escalation_to"), nte: amount == null ? undefined : { amountMinor: amount, currency: text(row, "nte_currency") || "USD" }, vendorServiceTicketNumber: maybeText(row, "vendor_service_ticket_number"), vendorInvoiceNumber: maybeText(row, "vendor_invoice_number"), externalAccountingPo: maybeText(row, "external_accounting_po"), createdAt: text(row, "created_at"), closedAt: maybeText(row, "closed_at") };
}
...
class D1OpsRepository implements OpsRepository {
  ...
  async findActiveVendorAssignment(organizationId: OpsId, workOrderId: OpsId, vendorId: OpsId) { const row = await this.first("SELECT * FROM ops_work_order_assignments WHERE organization_id = ? AND work_order_id = ? AND vendor_id = ? AND status NOT IN ('cancelled','declined','completed','superseded') ORDER BY assigned_at DESC LIMIT 1", [organizationId, workOrderId, vendorId]); return row ? assignmentFrom(row) : null; }
  ...
  async getStoreVisitContextByToken(input: PublicTokenLookup): Promise<StoreVisitContextView|null> { const gateway=await this.getPublicStoreGatewayByToken(input); if(!gateway||!input.vendorId||!gateway.approvedVendors.some(row=>row.id===input.vendorId))return null; const vendor=await this.getVendor(gateway.organizationId,input.vendorId); if(!vendor)return null; const rows=await this.all("SELECT w.id,w.number,w.problem,w.category_key,w.status FROM ops_work_orders w JOIN ops_work_order_assignments a ON a.organization_id=w.organization_id AND a.work_order_id=w.id WHERE w.organization_id=? AND w.store_id=? AND a.vendor_id=? AND a.status NOT IN ('cancelled','declined','completed','superseded') AND w.status NOT IN ('closed','cancelled') ORDER BY w.created_at DESC",[gateway.organizationId,gateway.store.id,vendor.id]); return {organizationName:gateway.organizationName,store:gateway.store,vendor:{id:vendor.id,name:vendor.name},eligibleWorkOrders:rows.map(row=>({id:text(row,"id"),number:text(row,"number"),problem:text(row,"problem"),categoryKey:maybeText(row,"category_key"),status:text(row,"status") as WorkOrder["status"]})),allowsNoWorkOrder:true}; }
  async getServiceAuthorizationByToken(input: PublicTokenLookup): Promise<ServiceAuthorizationView|null> { const token=await this.publicToken(input); if(!token||text(token,"subject_type")!=="work_order_issuance")return null; const org=text(token,"organization_id"); const issuance=await this.getIssuance(org,text(token,"subject_id")); if(!issuance)return null; const assignment=await this.getAssignment(org,issuance.assignmentId); const workOrder=await this.getWorkOrder(org,issuance.workOrderId); if(!assignment||!workOrder||assignment.kind!=="outside_vendor"||assignment.workOrderId!==workOrder.id||!assignment.vendorId)return null; const vendor=await this.getVendor(org,assignment.vendorId); const store=await this.getStore(org,workOrder.storeId); const organization=await this.getOrganization(org); if(!vendor||!store||!organization)return null; let snapshot:Partial<ServiceAuthorizationView>={}; try{snapshot=JSON.parse(issuance.immutablePayloadJson) as Partial<ServiceAuthorizationView>;}catch{return null;} const response=await this.first("SELECT * FROM ops_vendor_responses WHERE organization_id=? AND issuance_id=? ORDER BY responded_at DESC LIMIT 1",[org,issuance.id]); return {organizationId:org,organizationName:snapshot.organizationName??organization.name,workOrderId:workOrder.id,assignmentId:assignment.id,issuanceId:issuance.id,workOrderNumber:snapshot.workOrderNumber??workOrder.number,revision:issuance.revision,status:workOrder.status,store:snapshot.store??{id:store.id,storeNumber:store.storeNumber,name:store.name,formattedAddress:formatAddress(store)},vendor:snapshot.vendor??{id:vendor.id,name:vendor.name},problem:snapshot.problem??workOrder.problem,priority:snapshot.priority??workOrder.priority,authorizedScope:snapshot.authorizedScope,categoryKey:snapshot.categoryKey,asset:snapshot.asset,requestedTiming:snapshot.requestedTiming,nte:snapshot.nte,billingInstruction:snapshot.billingInstruction??`Reference operator work order ${workOrder.number} on all service tickets and invoices.`,issuedAt:issuance.issuedAt,latestResponse:response?{response:text(response,"response") as VendorResponse["response"],responderName:text(response,"responder_name"),proposedAt:maybeText(response,"proposed_at"),message:maybeText(response,"message"),respondedAt:text(response,"responded_at")}:undefined}; }
