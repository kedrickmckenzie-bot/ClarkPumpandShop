...
class D1OpsRepository implements OpsRepository {
  ...
  async listNotificationRules(organizationId: OpsId) { return (await this.all("SELECT * FROM ops_notification_rules WHERE organization_id = ? ORDER BY event_key", [organizationId])).map(notificationRuleFrom); }
  async upsertNotificationRule(input: Parameters<OpsRepository["upsertNotificationRule"]>[0]) { await this.atomicWrite([{ sql: "INSERT INTO ops_notification_rules (id, organization_id, event_key, email_enabled, recipient_role, updated_by_membership_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (organization_id, event_key) DO UPDATE SET email_enabled = excluded.email_enabled, recipient_role = excluded.recipient_role, updated_by_membership_id = excluded.updated_by_membership_id, updated_at = excluded.updated_at", params: [input.id, input.organizationId, input.eventKey, input.emailEnabled ? 1 : 0, input.recipientRole, input.updatedByMembershipId ?? null, input.occurredAt, input.occurredAt] }]); }
  async listNotificationRecipients(organizationId: OpsId, role: NotificationRule["recipientRole"]): Promise<NotificationRecipient[]> { const rows = await this.all("SELECT m.id AS membership_id, m.user_id, m.role, u.email, u.display_name FROM ops_memberships m JOIN ops_users u ON u.id = m.user_id WHERE m.organization_id = ? AND m.role = ? AND m.status = 'active' AND u.status = 'active' ORDER BY u.display_name, u.email", [organizationId, role]); return rows.map((row) => ({ membershipId: text(row, "membership_id"), userId: text(row, "user_id"), role: text(row, "role") as NotificationRecipient["role"], email: text(row, "email"), displayName: text(row, "display_name") })); }
